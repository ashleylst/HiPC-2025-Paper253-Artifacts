...
void vector_double_scale(vector_double z, vector_double x, complex_double alpha, int start, int end,
  ...
  __m128d alpha_re = _mm_set1_pd(real(alpha));
  __m128d alpha_im = _mm_set1_pd(imag(alpha));
...
void vector_double_scale(vector_double z, vector_double x, complex_double alpha, int start, int end,
  ...
  __m128 alpha_re = _mm_set1_ps(real(alpha));
  __m128 alpha_im = _mm_set1_ps(imag(alpha));
...
complex_double global_inner_product_double(vector_double phi, vector_double psi, int start, int end,
  ...
  int thread_start;
  int thread_end;
  compute_core_start_end(start, end, &thread_start, &thread_end, l);
...
void vector_double_multi_saxpy(vector_double z, vector_double *V, complex_double *alpha, int sign,
  ...
  int flag = 0;
  __m128d alpha_re[count];
  __m128d alpha_im[count];
  for (int c = 0; c < count; c++) {
    alpha_re[c] = _mm_set1_pd(sign * real(alpha[c]));
    alpha_im[c] = _mm_set1_pd(sign * imag(alpha[c]));
    if (imag(alpha[c]) > EPS_double || -imag(alpha[c]) > EPS_double)
      flag = 1;
  }
...
void vector_double_multi_saxpy(vector_double z, vector_double *V, complex_double *alpha, int sign,
  ...
  __m128 z_re;
  __m128 z_im;
  __m128 alpha_re[count];
  __m128 alpha_im[count];
  int flag = 0;
  ...
  for (int c = 0; c < count; c++) {
    alpha_re[c] = _mm_set1_ps(sign * real(alpha[c]));
    alpha_im[c] = _mm_set1_ps(sign * imag(alpha[c]));
    if (imag(alpha[c]) > EPS_double || -imag(alpha[c]) > EPS_double)
      flag = 1;
  }
...
void process_multi_inner_product_MP(int count, complex_double *results, vector_double *phi,
  ...
  compute_core_start_end_custom(start, end, &thread_start, &thread_end, l, 12);
  ...
  for (int c = 0; c < count; c++)
    for (int i = 1; i < threading->n_core; i++)
      ((complex_double **)threading->workspace)[0][c] +=
          ((complex_double **)threading->workspace)[i][c];
...
void process_multi_inner_product_double(int count, complex_double *results, vector_double *phi,
  ...
  if (l->depth == 0) {
    ...
    compute_core_start_end_custom(start, end, &thread_start, &thread_end, l, 4);
    for (int c = 0; c < count; c++) {
      __m128 result_re = _mm_setzero_ps();
      __m128 result_im = _mm_setzero_ps();
  ...
  PROF_double_STOP(_PIP, (double)(count * (end - start)) / (double)l->inner_vector_size);
}
#endif
