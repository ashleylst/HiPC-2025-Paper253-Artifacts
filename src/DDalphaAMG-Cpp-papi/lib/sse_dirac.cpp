...
#define index_re(phi, mu, spin)                                                                    \
  (gamma_re_sign[mu][spin]) * (phi)[6 * gamma_co[mu][spin] + gamma_offset[mu][spin]]
#define index_im(phi, mu, spin)                                                                    \
  (gamma_im_sign[mu][spin]) * (phi)[6 * gamma_co[mu][spin] - gamma_offset[mu][spin] + 1]

// 12*(gamma_co[mu][spin]/2) equivalent to #define flav_gamma(k) ((k)>1?((k)*3+6):((k)*3))
#define index_d_re(phi, mu, spin)                                                                  \
  (gamma_re_sign[mu][spin]) *                                                                      \
      (phi)[6 * gamma_co[mu][spin] + 12 * (gamma_co[mu][spin] / 2) + gamma_offset[mu][spin]]
#define index_d_im(phi, mu, spin)                                                                  \
  (gamma_im_sign[mu][spin]) *                                                                      \
      (phi)[6 * gamma_co[mu][spin] + 12 * (gamma_co[mu][spin] / 2) - gamma_offset[mu][spin] + 1]
...
void prp_double(complex_double *prn[4], complex_double *phi, int start, int end) {
  ...
  double *pr[4] = {(double *)(prn[0] + start / 2), (double *)(prn[1] + start / 2),
                   (double *)(prn[2] + start / 2), (double *)(prn[3] + start / 2)};
...
void dprp_double(complex_double *prn[4], complex_double *phi, int start, int end) {
  ...
  while (phi_pt < phi_end) {
    ...
    for (int mu = 0; mu < 4; mu++) {
      __m128d phi_pt2_re =
          _mm_setr_pd(index_d_re(phi_pt + 2, mu, 1), index_d_re(phi_pt + 4, mu, 1));
      __m128d phi_pt2_im =
          _mm_setr_pd(index_d_im(phi_pt + 2, mu, 1), index_d_im(phi_pt + 4, mu, 1));
      __m128d res_re = _mm_sub_pd(phi_pt1_re, phi_pt2_re);
      __m128d res_im = _mm_sub_pd(phi_pt1_im, phi_pt2_im);
      sse_complex_interleaved_store_pd(res_re, res_im, pr[mu]);
      pr[mu] += 2 * SIMD_LENGTH_double;
    ...
    for (int mu = 0; mu < 4; mu++) {
      __m128d phi_pt2_re =
          _mm_setr_pd(index_d_re(phi_pt + 12, mu, 0), index_d_re(phi_pt + 14, mu, 0));
      __m128d phi_pt2_im =
          _mm_setr_pd(index_d_im(phi_pt + 12, mu, 0), index_d_im(phi_pt + 14, mu, 0));
      __m128d res_re = _mm_sub_pd(phi_pt1_re, phi_pt2_re);
      __m128d res_im = _mm_sub_pd(phi_pt1_im, phi_pt2_im);
      sse_complex_interleaved_store_pd(res_re, res_im, pr[mu]);
      pr[mu] += 2 * SIMD_LENGTH_double;
    ...
    for (int mu = 0; mu < 4; mu++) {
      __m128d phi_pt2_re =
          _mm_setr_pd(index_d_re(phi_pt + 16, mu, 0), index_d_re(phi_pt + 12, mu, 1));
      __m128d phi_pt2_im =
          _mm_setr_pd(index_d_im(phi_pt + 16, mu, 0), index_d_im(phi_pt + 12, mu, 1));
      __m128d res_re = _mm_sub_pd(phi_pt1_re, phi_pt2_re);
      __m128d res_im = _mm_sub_pd(phi_pt1_im, phi_pt2_im);
      sse_complex_interleaved_store_pd(res_re, res_im, pr[mu]);
      pr[mu] += 2 * SIMD_LENGTH_double;
    ...
    for (int mu = 0; mu < 4; mu++) {
      __m128d phi_pt2_re =
          _mm_setr_pd(index_d_re(phi_pt + 14, mu, 1), index_d_re(phi_pt + 16, mu, 1));
      __m128d phi_pt2_im =
          _mm_setr_pd(index_d_im(phi_pt + 14, mu, 1), index_d_im(phi_pt + 16, mu, 1));
      __m128d res_re = _mm_sub_pd(phi_pt1_re, phi_pt2_re);
      __m128d res_im = _mm_sub_pd(phi_pt1_im, phi_pt2_im);
      sse_complex_interleaved_store_pd(res_re, res_im, pr[mu]);
      pr[mu] += 2 * SIMD_LENGTH_double;
...
void dprp_double(complex_double *prn[4], complex_double *phi, int start, int end) {
  ...
  while (phi_pt < phi_end) {
    ...
    for (int mu = 0; mu < 4; mu++) {
      __m128 phi_pt2_re =
          _mm_setr_ps(index_d_re(phi_pt + 2, mu, 1), index_d_re(phi_pt + 4, mu, 1),
                      index_d_re(phi_pt + 12, mu, 0), index_d_re(phi_pt + 14, mu, 0));
      __m128 phi_pt2_im =
          _mm_setr_ps(index_d_im(phi_pt + 2, mu, 1), index_d_im(phi_pt + 4, mu, 1),
    ...
    for (int mu = 0; mu < 4; mu++) {
      __m128 phi_pt2_re =
          _mm_setr_ps(index_d_re(phi_pt + 16, mu, 0), index_d_re(phi_pt + 12, mu, 1),
                      index_d_re(phi_pt + 14, mu, 1), index_d_re(phi_pt + 16, mu, 1));
      __m128 phi_pt2_im =
          _mm_setr_ps(index_d_im(phi_pt + 16, mu, 0), index_d_im(phi_pt + 12, mu, 1),
...
void prn_su3_double(complex_double *prp[4], complex_double *phi, operator_double_struct *op,
  ...
  while (phi_pt < phi_end_pt) {
    ...
    for (int mu = 0; mu < 4; mu++) {
...

      {
        ...
        res1[0] = _mm_setr_ps(index_re(phi_pt, mu, 0), index_re(phi_pt + 2, mu, 0),
                              index_re(phi_pt + 4, mu, 0), 0);
        res1[1] = _mm_setr_ps(index_im(phi_pt, mu, 0), index_im(phi_pt + 2, mu, 0),
                              index_im(phi_pt + 4, mu, 0), 0);
        __m128 in1_re = _mm_add_ps(in1[0], res1[0]);
        __m128 in1_im = _mm_add_ps(in1[1], res1[1]);

        // calc spin1 projection
        res1[0] = _mm_setr_ps(index_re(phi_pt, mu, 1), index_re(phi_pt + 2, mu, 1),
                              index_re(phi_pt + 4, mu, 1), 0);
        res1[1] = _mm_setr_ps(index_im(phi_pt, mu, 1), index_im(phi_pt + 2, mu, 1),
                              index_im(phi_pt + 4, mu, 1), 0);
...
void dprn_su3_double(complex_double *prp[4], complex_double *phi, operator_double_struct *op,
                     int *neighbor, int start, int end) {
  ...
  while (phi_pt < phi_end_pt) {
    ...
    for (int mu = 0; mu < 4; mu++) {
      ...
      for (int i = 3; i < 6; i++) {
        v_re[i] = _mm_setr_pd(index_d_re(phi_pt + 6 + 2 * i, mu, 0),
                              index_d_re(phi_pt + 6 + 2 * i, mu, 1));
        v_im[i] = _mm_setr_pd(index_d_im(phi_pt + 6 + 2 * i, mu, 0),
                              index_d_im(phi_pt + 6 + 2 * i, mu, 1));
        v_re[i] = _mm_add_pd(in_re[i], v_re[i]);
        v_im[i] = _mm_add_pd(in_im[i], v_im[i]);
      }
...
void dprn_su3_double(complex_double *prp[4], complex_double *phi, operator_double_struct *op,
  ...
  while (phi_pt < phi_end_pt) {
    ...
    for (int mu = 0; mu < 4; mu++) {
      ...
      {
        // calc spin0 projection
        res11[0] = _mm_setr_ps(index_d_re(phi_pt, mu, 0), index_d_re(phi_pt + 2, mu, 0),
                               index_d_re(phi_pt + 4, mu, 0), 0);
        res11[1] = _mm_setr_ps(index_d_im(phi_pt, mu, 0), index_d_im(phi_pt + 2, mu, 0),
                               index_d_im(phi_pt + 4, mu, 0), 0);
        __m128 in11_re = _mm_add_ps(in11[0], res11[0]);
        __m128 in11_im = _mm_add_ps(in11[1], res11[1]);

        // calc spin1 projection
        res11[0] = _mm_setr_ps(index_d_re(phi_pt, mu, 1), index_d_re(phi_pt + 2, mu, 1),
                               index_d_re(phi_pt + 4, mu, 1), 0);
        res11[1] = _mm_setr_ps(index_d_im(phi_pt, mu, 1), index_d_im(phi_pt + 2, mu, 1),
                               index_d_im(phi_pt + 4, mu, 1), 0);
        __m128 in21_re = _mm_add_ps(in21[0], res11[0]);
        __m128 in21_im = _mm_add_ps(in21[1], res11[1]);

        // calc spin0 projection
        res12[0] = _mm_setr_ps(index_d_re(phi_pt + 12, mu, 0), index_d_re(phi_pt + 14, mu, 0),
                               index_d_re(phi_pt + 16, mu, 0), 0);
        res12[1] = _mm_setr_ps(index_d_im(phi_pt + 12, mu, 0), index_d_im(phi_pt + 14, mu, 0),
                               index_d_im(phi_pt + 16, mu, 0), 0);
        __m128 in12_re = _mm_add_ps(in12[0], res12[0]);
        __m128 in12_im = _mm_add_ps(in12[1], res12[1]);

        // calc spin1 projection
        res12[0] = _mm_setr_ps(index_d_re(phi_pt + 12, mu, 1), index_d_re(phi_pt + 14, mu, 1),
                               index_d_re(phi_pt + 16, mu, 1), 0);
        res12[1] = _mm_setr_ps(index_d_im(phi_pt + 12, mu, 1), index_d_im(phi_pt + 14, mu, 1),
                               index_d_im(phi_pt + 16, mu, 1), 0);
...
void pbn_double(complex_double *eta, complex_double *prp[4], int start, int end) {
  ...
  for (int mu = 0; mu < 4; mu++) {
    gamma0[mu] =
        _mm_setr_pd(gamma_re_sign[mu][gamma_co[mu][0]], gamma_im_sign[mu][gamma_co[mu][0]]);
    gamma1[mu] =
        _mm_setr_pd(gamma_re_sign[mu][gamma_co[mu][1]], gamma_im_sign[mu][gamma_co[mu][1]]);
  }
...
void dpbn_double(complex_double *eta, complex_double *prp[4], int start, int end) {
  ...
  for (int i = start; i < end; i += 24) {
    ...
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[T], GAMMA_T_SHUFFLE(in[j]));
      res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[T], GAMMA_T_SHUFFLE(in[j + 3]));
      res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[T], GAMMA_T_SHUFFLE(in[j + 6]));
      res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + 6 + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[T], GAMMA_T_SHUFFLE(in[j + 9]));
      res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + 6 + j], buf1);
    }
    // ---------------
    // mu = Z
    for (int j = 0; j < 12; j++) {
    ...
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[Z], GAMMA_Z_SHUFFLE(in[j]));
      res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[Z], GAMMA_Z_SHUFFLE(in[j + 3]));
      res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[Z], GAMMA_Z_SHUFFLE(in[j + 6]));
      res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + 6 + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[Z], GAMMA_Z_SHUFFLE(in[j + 9]));
      res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + 6 + j], buf1);
    }
    // ---------------
    // mu = Y
    for (int j = 0; j < 12; j++) {
    ...
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[Y], GAMMA_Y_SHUFFLE(in[j]));
      res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[Y], GAMMA_Y_SHUFFLE(in[j + 3]));
      res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[Y], GAMMA_Y_SHUFFLE(in[j + 6]));
      res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + 6 + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[Y], GAMMA_Y_SHUFFLE(in[j + 9]));
      res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + 6 + j], buf1);
    }
    // ---------------
    // mu = X
    for (int j = 0; j < 12; j++) {
    ...
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[X], GAMMA_X_SHUFFLE(in[j]));
      res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[X], GAMMA_X_SHUFFLE(in[j + 3]));
      res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + j] =
          _mm_sub_pd(res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma0[X], GAMMA_X_SHUFFLE(in[j + 6]));
      res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + 6 + j], buf1);
    }
    for (int j = 0; j < 3; j++) {
      __m128d buf1 = _mm_mul_pd(gamma1[X], GAMMA_X_SHUFFLE(in[j + 9]));
      res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + 6 + j] =
          _mm_sub_pd(res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + 6 + j], buf1);
    }
    // ---------------
  ...
  while (eta_pt < eta_end_pt) {
    ...
    // mu = T
    {
      ...
      }
      {
        ...
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[T], GAMMA_T_SHUFFLE(in[i]));
          res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[T], GAMMA_T_SHUFFLE(in[i + 3]));
          res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[T], GAMMA_T_SHUFFLE(in[i + 6]));
          res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[T][0] + 6 * (gamma_co[T][0] / 2) + 6 + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[T], GAMMA_T_SHUFFLE(in[i + 9]));
          res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[T][1] + 6 * (gamma_co[T][1] / 2) + 6 + i], buf1);
        }
      }
    }
    // ---------------
    // mu = Z
    {
      ...
      }
      {
        ...
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[Z], GAMMA_Z_SHUFFLE(in[i]));
          res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[Z], GAMMA_Z_SHUFFLE(in[i + 3]));
          res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[Z], GAMMA_Z_SHUFFLE(in[i + 6]));
          res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[Z][0] + 6 * (gamma_co[Z][0] / 2) + 6 + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[Z], GAMMA_Z_SHUFFLE(in[i + 9]));
          res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[Z][1] + 6 * (gamma_co[Z][1] / 2) + 6 + i], buf1);
        }
      }
    }
    // ---------------
    // mu = Y
    {
      ...
      }
      {
        ...
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[Y], GAMMA_Y_SHUFFLE(in[i]));
          res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[Y], GAMMA_Y_SHUFFLE(in[i + 3]));
          res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[Y], GAMMA_Y_SHUFFLE(in[i + 6]));
          res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[Y][0] + 6 * (gamma_co[Y][0] / 2) + 6 + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[Y], GAMMA_Y_SHUFFLE(in[i + 9]));
          res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[Y][1] + 6 * (gamma_co[Y][1] / 2) + 6 + i], buf1);
        }
      }
    }
    // ---------------
    // mu = X
    {
      ...
      }
      {
        ...
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[X], GAMMA_X_SHUFFLE(in[i]));
          res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[X], GAMMA_X_SHUFFLE(in[i + 3]));
          res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + i] =
              _mm_sub_pd(res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma0[X], GAMMA_X_SHUFFLE(in[i + 6]));
          res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[X][0] + 6 * (gamma_co[X][0] / 2) + 6 + i], buf1);
        }
        for (int i = 0; i < 3; i++) {
          __m128d buf1 = _mm_mul_pd(gamma1[X], GAMMA_X_SHUFFLE(in[i + 9]));
          res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + 6 + i] =
              _mm_sub_pd(res[3 * gamma_co[X][1] + 6 * (gamma_co[X][1] / 2) + 6 + i], buf1);
        }
      }
    }
    // ---------------
...
void block_oddeven_plus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
                                        int end, int *ind, int *neighbor) {}
void block_oddeven_pT_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
...
void block_oddeven_pZ_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 1
#include neighbor_coupling_file
...
void block_oddeven_pY_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 2
#include neighbor_coupling_file
...
void block_oddeven_pX_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 3
#include neighbor_coupling_file
...
void block_oddeven_plus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
  ...
  switch (mu) {
  case T:
    block_oddeven_pT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    block_oddeven_pZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    block_oddeven_pY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    block_oddeven_pX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("block_oddeven_plus_coupling_double: invalid mu=%d\n", mu);
  }
}

void block_oddeven_nplus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
                                         int end, int *ind, int *neighbor) {}
void block_oddeven_npT_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
...
void block_oddeven_npZ_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 1
#include neighbor_coupling_file
...
void block_oddeven_npY_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 2
#include neighbor_coupling_file
...
void block_oddeven_npX_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 3
#include neighbor_coupling_file
...
void block_oddeven_nplus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
  ...
  switch (mu) {
  case T:
    block_oddeven_npT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    block_oddeven_npZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    block_oddeven_npY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    block_oddeven_npX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("block_oddeven_nplus_coupling_double: invalid mu=%d\n", mu);
  }
}

void block_oddeven_minus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
                                         int end, int *ind, int *neighbor) {}
void block_oddeven_mT_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 0
#define MINUSDIR
...
void block_oddeven_mZ_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 1
#define MINUSDIR
...
void block_oddeven_mY_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 2
#define MINUSDIR
...
void block_oddeven_mX_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                      int *ind, int *neighbor) {
#define UPD _mm_sub_ps
#define MU 3
#define MINUSDIR
...
void block_oddeven_minus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
  ...
  switch (mu) {
  case T:
    block_oddeven_mT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    block_oddeven_mZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    block_oddeven_mY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    block_oddeven_mX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("block_oddeven_minus_coupling_double: invalid mu=%d\n", mu);
  }
}

void block_oddeven_nminus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
                                          int end, int *ind, int *neighbor) {}
void block_oddeven_nmT_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 0
#define MINUSDIR
...
void block_oddeven_nmZ_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 1
#define MINUSDIR
...
void block_oddeven_nmY_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 2
#define MINUSDIR
...
void block_oddeven_nmX_coupling_double(double *eta, double *D, double *phi, int start, int end,
                                       int *ind, int *neighbor) {
#define UPD _mm_add_ps
#define MU 3
#define MINUSDIR
...
void block_oddeven_nminus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
  ...
  switch (mu) {
  case T:
    block_oddeven_nmT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    block_oddeven_nmZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    block_oddeven_nmY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    block_oddeven_nmX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("block_oddeven_nminus_coupling_double: invalid mu=%d\n", mu);
  }
}

void boundary_nminus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
                                     int end, int *ind, int *neighbor) {}
void boundary_nmT_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 0
#define BOUNDARY
...
void boundary_nmZ_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 1
#define BOUNDARY
...
void boundary_nmY_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 2
#define BOUNDARY
...
void boundary_nmX_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 3
#define BOUNDARY
...
void boundary_nminus_coupling_double(double *eta, double *D, double *phi, int mu, int start,
  ...
  switch (mu) {
  case T:
    boundary_nmT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    boundary_nmZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    boundary_nmY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    boundary_nmX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("boundary_nminus_coupling_double: invalid mu=%d\n", mu);
  }
}

void boundary_nplus_coupling_double(double *eta, double *D, double *phi, int mu, int start, int end,
                                    int *ind, int *neighbor) {}
void boundary_npT_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 0
#define BOUNDARY
...
void boundary_npZ_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 1
#define BOUNDARY
...
void boundary_npY_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 2
#define BOUNDARY
...
void boundary_npX_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                  int *neighbor) {
#define UPD _mm_add_ps
#define MU 3
#define BOUNDARY
...
void boundary_nplus_coupling_double(double *eta, double *D, double *phi, int mu, int start, int end,
  ...
  switch (mu) {
  case T:
    boundary_npT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    boundary_npZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    boundary_npY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    boundary_npX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("boundary_nplus_coupling_double: invalid mu=%d\n", mu);
  }
}

void boundary_minus_coupling_double(double *eta, double *D, double *phi, int mu, int start, int end,
                                    int *ind, int *neighbor) {}
void boundary_mT_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 0
#define BOUNDARY
...
void boundary_mZ_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 1
#define BOUNDARY
...
void boundary_mY_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 2
#define BOUNDARY
...
void boundary_mX_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 3
#define BOUNDARY
...
void boundary_minus_coupling_double(double *eta, double *D, double *phi, int mu, int start, int end,
  ...
  switch (mu) {
  case T:
    boundary_mT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    boundary_mZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    boundary_mY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    boundary_mX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("boundary_minus_coupling_double: invalid mu=%d\n", mu);
  }
}

void boundary_plus_coupling_double(double *eta, double *D, double *phi, int mu, int start, int end,
                                   int *ind, int *neighbor) {}
void boundary_pT_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 0
#define BOUNDARY
...
void boundary_pZ_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 1
#define BOUNDARY
...
void boundary_pY_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 2
#define BOUNDARY
...
void boundary_pX_coupling_double(double *eta, double *D, double *phi, int start, int end, int *ind,
                                 int *neighbor) {
#define UPD _mm_sub_ps
#define MU 3
#define BOUNDARY
...
void boundary_plus_coupling_double(double *eta, double *D, double *phi, int mu, int start, int end,
  ...
  switch (mu) {
  case T:
    boundary_pT_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Z:
    boundary_pZ_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case Y:
    boundary_pY_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  case X:
    boundary_pX_coupling_double(eta, D, phi, start, end, ind, neighbor);
    break;
  default:
    error0("boundary_plus_coupling_double: invalid mu=%d\n", mu);
  }
}

static inline int sse_clover_real_index(int i, int j) {
  return (i / SIMD_LENGTH_double) * 12 * SIMD_LENGTH_double + SIMD_LENGTH_double * j * 2 +
         i % SIMD_LENGTH_double;
}

static inline int sse_clover_imag_index(int i, int j) {
  return (i / SIMD_LENGTH_double) * 12 * SIMD_LENGTH_double + SIMD_LENGTH_double * (j * 2 + 1) +
         i % SIMD_LENGTH_double;
}
      ...
      for (int i = 0; i < SIMD_LENGTH_double; i++) {
        if (i + k == j || i + k - 6 == j) {
          ...
          if (j > i + k - 6) {
            // upper triangle
            index =
                12 + 15 + (30 - (5 - (k + i - 6)) * (6 - (k + i - 6))) / 2 + (j - (i + k - 6 + 1));
            sign = 1.0;
          } else {
            // j < i+k-6
            // lower triangle
        ...
        out[sse_clover_real_index(i + k, j)] = real((complex_double)in[index]);
        out[sse_clover_imag_index(i + k, j)] = sign * imag((complex_double)in[index]);
      }
    }
  }
}
  ...
  for (int k = 0; k < 12; k += SIMD_LENGTH_double) {
    for (int j = 0; j < 6; j++) {
      for (int i = 0; i < SIMD_LENGTH_double; i++) {
        ...
        d = (i + k < 6) ? 0 : 6;
        out[sse_clover_real_index(i + k + d, j)] = real(in[index]);
        out[sse_clover_imag_index(i + k + d, j)] = sign * imag(in[index]);
        out[sse_clover_real_index(i + k + d + 6, j)] = real(in[index]);
        out[sse_clover_imag_index(i + k + d + 6, j)] = sign * imag(in[index]);
      }
    }
  }
}
...
void sse_add_diagonal_clover_double(double *out, complex_double *diag) {
  for (int k = 0; k < 12; k++) {
    out[sse_clover_real_index(k, k % 6)] += real(diag[k]);
    out[sse_clover_imag_index(k, k % 6)] += imag(diag[k]);
  }
}
...
void sse_add_diagonal_clover_doublet_double(double *out, complex_double *diag) {
  for (int k = 0; k < 6; k++) {
    out[sse_clover_real_index(k, k % 6)] += real(diag[k]);
    out[sse_clover_imag_index(k, k % 6)] += imag(diag[k]);
    out[sse_clover_real_index(k + 6, k % 6)] -= real(diag[k]);
    out[sse_clover_imag_index(k + 6, k % 6)] -= imag(diag[k]);
  }
  for (int k = 6; k < 12; k++) {
    out[sse_clover_real_index(k + 6, k % 6)] += real(diag[k]);
    out[sse_clover_imag_index(k + 6, k % 6)] += imag(diag[k]);
    out[sse_clover_real_index(k + 12, k % 6)] -= real(diag[k]);
    out[sse_clover_imag_index(k + 12, k % 6)] -= imag(diag[k]);
  }
}
...
void sse_site_clover_doublet_invert_double(double *clover_in, config_double eps_term,
                                           double *clover_out) {}
...
void sse_site_clover_doublet_invert_double(double *clover_in, config_double eps_term,
  ...
  for (int n = 0; n < 2; n++)
    for (int j = 0; j < 6; j++) {
      M_tmp[288 * n + 24 * (j + 6) + j] = M_tmp[288 * n + 24 * j + j + 6] =
          real(eps_term[6 * n + j]);
      M_tmp[288 * n + 24 * (j + 6) + j + 12] = M_tmp[288 * n + 24 * j + j + 18] =
          cimag(eps_term[6 * n + j]);
    }
