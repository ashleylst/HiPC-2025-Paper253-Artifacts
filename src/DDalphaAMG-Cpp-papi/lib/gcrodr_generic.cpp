...
int fgmresx_double(gmres_double_struct *, level *);
void gev_buildAB_double(complex_double **, complex_double **, complex_double **, vector_double *,
                        vector_double *, int, gmres_double_struct *, level *);
void order_pairs_double(vector_double, complex_double *, int *, int);
void build_CU_double(complex_double **, vector_double *, vector_double *, gmres_double_struct *,
                     level *, int);
int arnoldix_step_double(vector_double *V, vector_double *Z, vector_double w, complex_double **H,
                         complex_double *buffer, int j, void (*prec)(), gmres_double_struct *p,
                         Level *l);
void re_scale_Uk_double(gmres_double_struct *p, Level *l);
...
void flgcrodr_double_struct_init(gmres_double_struct *p) {
  ...
  p->gcrodr_double.eigslvr.ordr_idxs = nullptr;
  p->gcrodr_double.eigslvr.ordr_keyscpy = nullptr;

  p->gcrodr_double.C = nullptr;
  p->gcrodr_double.Cc = nullptr;
  p->gcrodr_double.U = nullptr;
  p->gcrodr_double.gev_A = nullptr;
  p->gcrodr_double.gev_B = nullptr;
  p->gcrodr_double.Bbuff = nullptr;
  p->gcrodr_double.eigslvr.w = nullptr;
  p->gcrodr_double.eigslvr.beta = nullptr;
  p->gcrodr_double.eigslvr.vl = nullptr;
  p->gcrodr_double.eigslvr.vr = nullptr;
  p->gcrodr_double.eigslvr.qr_tau = nullptr;

  p->gcrodr_double.Yk = nullptr;
  p->gcrodr_double.Pk = nullptr;

  p->gcrodr_double.QR = nullptr;
  p->gcrodr_double.Q = nullptr;
  p->gcrodr_double.R = nullptr;
  p->gcrodr_double.Rinv = nullptr;

  p->gcrodr_double.ort_B = nullptr;
  p->gcrodr_double.G = nullptr;
  p->gcrodr_double.Gc = nullptr;
  p->gcrodr_double.hatZ = nullptr;
  p->gcrodr_double.hatW = nullptr;
#ifdef BLOCK_JACOBI
  p->gcrodr_double.r_aux = nullptr;
#endif

#if defined(SINGLE_ALLREDUCE_ARNOLDI) && defined(PIPELINED_ARNOLDI)
  p->gcrodr_double.PC = nullptr;
  p->gcrodr_double.DPC = nullptr;
#endif
}

void flgcrodr_double_struct_alloc(int m, int n, long int vl, double tol, const int type,
  ...
  if (l->currentLevel == 0) {
    ...
    p->gcrodr_double.Bbuff[0] = nullptr;
    ...
    p->gcrodr_double.gev_A[0] = nullptr;
    p->gcrodr_double.gev_B[0] = nullptr;
    ...
    p->gcrodr_double.ort_B[0] = nullptr;
...
void flgcrodr_double_struct_free(gmres_double_struct *p, Level *l) {
...
int flgcrodr_double(gmres_double_struct *p, Level *l) {
  ...
  beta = global_norm_double(p->r, p->v_start, p->v_end, l); // gamma_0 = norm(r)
  norm_r0 = beta;
  ...
  if (!p->initial_guess_zero) {
    p->gcrodr_double.b_norm = global_norm_double(p->b, p->v_start, p->v_end, l);
  } else {
    START_MASTER(threading)
  ...
  if (p->gcrodr_double.CU_usable == 1) {
    ...
    if (p->gcrodr_double.update_CU == 1) {
      ...
      // QR = A*Yk
      for (i = 0; i < k; i++) {
        apply_operator_double(p->gcrodr_double.C[i], p->gcrodr_double.Yk[i], p, l);
      }
      ...
      pqr_double(i_length, k, p->gcrodr_double.C, p->gcrodr_double.R, p, l);
      // re-orthogonalize ?
      // pqr_double( i_length, k, p->gcrodr_double.C, p->gcrodr_double.R, p, l );
    ...
    process_multi_inner_product_double(k, tmpx, Ck, p->r, p->v_start, p->v_end, l);
    START_MASTER(threading)
    ...
    if (g.num_processes > 1) {
      ...
      MPI_Allreduce(buffer, bf, k, MPI_COMPLEX_double, MPI_SUM,
                    (l->depth == 0) ? g.comm_cart : l->gs_double.level_comm);
    ...
    // TODO : restore (?) this portion of code, but with a flag that indicates that the first k
    // diagonal entries
    ...
    // compute \tilde{ Uk }
    for ( i=0; i<k; i++ ) {
      complex_double diag_term = global_norm_double( Uk[i], p->v_start, p->v_end, l );
      START_MASTER(threading)
    ...
    */
    ...
    compute_solution_double(p->x, (p->preconditioner && p->kind == _RIGHT) ? p->Z : p->V, p->y,
                            p->gamma, p->H, m - 1, 1, p, l);

#ifdef BLOCK_JACOBI

    // computing the actual residual in case of Block Jacobi
    {
      p->eval_operator(p->w, p->x, p->op, l); // compute w = D*x
      vector_double_minus(p->gcrodr_double.r_aux, p->block_jacobi_double.b_backup, p->w, start, end,
                          l); // compute r = b - w

      double norm_r0xx = global_norm_double(p->block_jacobi_double.b_backup, start, end, l);
      double betaxx = global_norm_double(p->gcrodr_double.r_aux, start, end, l);
    ...
    if (l->currentLevel == 0 && p->block_jacobi_double.BJ_usable == 1) {
      block_jacobi_apply_double(l->p_double.block_jacobi_double.xtmp, p->w, p, l);
      vector_double_minus(p->r, p->b, l->p_double.block_jacobi_double.xtmp, start, end,
                          l); // compute r = b - w
    } else {
      apply_operator_double(p->w, p->x, p, l);              // compute w = D*x
      vector_double_minus(p->r, p->b, p->w, start, end, l); // compute r = b - w
    ...
    if (p->preconditioner == nullptr) {
      ...
      gev_buildAB_double(p->gcrodr_double.gev_A, p->gcrodr_double.gev_B,
                         p->gcrodr_double.eigslvr.Hc, p->V, p->V, m, p, l);

      SYNC_MASTER_TO_ALL(threading);
      SYNC_CORES(threading)

      // build C and U
      build_CU_double(p->gcrodr_double.eigslvr.Hc, p->V, p->V, p, l, m);
    } else {
      // build the matrices A and B used for generalized-eigensolving
      gev_buildAB_double(p->gcrodr_double.gev_A, p->gcrodr_double.gev_B,
                         p->gcrodr_double.eigslvr.Hc, p->V, p->Z, m, p, l);

      SYNC_MASTER_TO_ALL(threading);
      SYNC_CORES(threading)

      // build C and U
      build_CU_double(p->gcrodr_double.eigslvr.Hc, p->V, p->Z, p, l, m);
    }
    ...
    error0("Invalid value for p->gcrodr_double.CU_usable \n");
  }
  ...
  for (ol = 0; ol < p->num_restart; ol++) {
    ...
    // set H within G
    for (j = 0; j < m; j++) {
      memcpy(p->gcrodr_double.G[k + j] + k, p->gcrodr_double.eigslvr.Hc[j],
             sizeof(complex_double) * (j + 2));
      memcpy(p->gcrodr_double.Gc[k + j] + k, p->gcrodr_double.eigslvr.Hc[j],
             sizeof(complex_double) * (j + 2));
    }
    END_MASTER(threading);
    ...
    // and, the last ingredient for the least squares problem is : <bf> as the rhs, <G> as the
    // matrix
    ...
    START_MASTER(threading)
    gels_double(LAPACK_COL_MAJOR, 'N', k + m + 1, k + m, 1, p->gcrodr_double.G[0],
                k + p->restart_length + 1, bf, k + p->restart_length + 1);
    END_MASTER(threading)
    ...
    for (i = 0; i < k; i++) {
      p->gcrodr_double.G[i][i] = p->gcrodr_double.Gc[i][i];
    }
    END_MASTER(threading)
    ...
    // update the solution
    for (i = 0; i < k; i++) {
      vector_double_saxpy(p->x, p->x, p->gcrodr_double.hatZ[i], bf[i] * p->gcrodr_double.Gc[i][i],
                          start, end, l);
    }
    for (i = k; i < (k + m); i++)
    ...
    if (p->gcrodr_double.upd_ctr < g.gcrodr_upd_itrs) {
      ...
      gev_buildAB_double(p->gcrodr_double.gev_A, p->gcrodr_double.gev_B, p->gcrodr_double.Gc,
                         p->gcrodr_double.hatW, p->gcrodr_double.hatZ, k + m, p, l);

      SYNC_MASTER_TO_ALL(threading);
      SYNC_CORES(threading)

      // build C and U
      build_CU_double(p->gcrodr_double.Gc, p->gcrodr_double.hatW, p->gcrodr_double.hatZ, p, l,
                      threading, k + m);
...
void gev_buildAB_double(complex_double **A, complex_double **B, complex_double **G,
                        ...
                        Level *l, struct Thread *threading) {
  ...
  for (j = 0; j < mk; j++) {
    process_multi_inner_product_double(mk + 1, tmpy + j * (mk + 1), W, Z[j], p->v_start, p->v_end,
                                       l);
  }
  ...
  if (g.num_processes > 1) {
    PROF_double_START(_ALLR);
    MPI_Allreduce(tmpy, Bbuffx, (mk + 1) * mk, MPI_COMPLEX_double, MPI_SUM,
                  (l->depth == 0) ? g.comm_cart : l->gs_double.level_comm);
    PROF_double_STOP(_ALLR, 1);
  } else {
    for (i = 0; i < (mk + 1) * mk; i++)
...
int fgmresx_double(gmres_double_struct *p, Level *l, struct Thread *threading) {
  ...
  if (l->currentLevel == 0 && g.num_levels > 1 && g.interpolation)
    p->tol = g.coarse_tol;
  ...
  if (l->currentLevel == 0 && l->depth > 0) {
    arnoldi_step_double(p->V, p->Z, p->w, p->H, p->y, 0, p->preconditioner, p, l);
  }
#endif
  ...
  for (il = 0; il < p->restart_length && finish == 0; il++) {
...
#if defined(SINGLE_ALLREDUCE_ARNOLDI) && defined(PIPELINED_ARNOLDI)
    if (l->currentLevel == 0 && l->depth > 0) {
      if (!arnoldi_step_double(p->V, p->Z, p->w, p->H, p->y, j + 1, p->preconditioner, p, l,
                               threading)) {
      ...
      if (!arnoldi_step_double(p->V, p->Z, p->w, p->H, p->y, j, p->preconditioner, p, l,
                               threading)) {
        printf0("| -------------- iteration %d, restart due to H(%d,%d) < 0 |\n", iter, j + 1, j);
        break;
      }
    ...
    if (abs(p->H[j][j + 1]) > p->tol / 10) {
      qr_update_double(p->H, p->s, p->c, p->gamma, j, l);
      SYNC_MASTER_TO_ALL(threading)
      SYNC_CORES(threading)
      gamma_jp1 = abs(p->gamma[j + 1]);
      ...
      if (gamma_jp1 / norm_r0 < p->tol || gamma_jp1 / norm_r0 > 1E+5) { // if satisfied ... stop
        ...
        START_MASTER(threading)
        if (gamma_jp1 / norm_r0 > 1E+5)
          printf0("Divergence of fgmresx_double, iter = %d, level=%d\n", iter, l->currentLevel);
        END_MASTER(threading)
      }
    } else {
      START_MASTER(threading)
...
void order_pairs_double(vector_double keys, complex_double *keys_cpy, int *output, int n) {
  ...
  for (i = 0; i < n; i++) {
    for (j = i + 1; j < n; j++) {
      if (abs(keys_cpy[i]) > abs(keys_cpy[j])) {
        buff2 = keys_cpy[i];
...
void build_CU_double(complex_double **G, vector_double *W, vector_double *Z, gmres_double_struct *p,
                     Level *l, struct Thread *threading, int m) {
  ...
  // p->gcrodr_double.eigslvr.ordr_idxs contains the indices to access w and vr in ascending
  // magnitude
  ...
  for (i = 0; i < k; i++) {
    ...
    if (p->gcrodr_double.CU_usable == 1) {
      for (j = 0; j < k; j++)
        vector_double_saxpy(Yk[i], Yk[i], Z[j], Pk[i][j] * G[j][j], start, end, l);
      SYNC_MASTER_TO_ALL(threading);
      SYNC_CORES(threading)
  ...
  for (i = 0; i < k; i++) {
    process_multi_inner_product_double(1, tmpy + i, &(Uk[i]), Uk[i], p->v_start, p->v_end, l,
                                       threading);
  }
  ...
  if (g.num_processes > 1) {
    PROF_double_START(_ALLR);
    MPI_Allreduce(tmpy, Bbuff[0], k, MPI_COMPLEX_double, MPI_SUM,
                  (l->depth == 0) ? g.comm_cart : l->gs_double.level_comm);
    PROF_double_STOP(_ALLR, 1);
  } else {
    for (i = 0; i < k; i++)
  ...
  for (i = 0; i < k; i++) {
    complex_double diag_term = sqrt(real(Bbuff[0][i]));
    START_MASTER(threading)
    diag_term = 1.0 / diag_term;
    p->gcrodr_double.G[i][i] = diag_term;
    p->gcrodr_double.Gc[i][i] = diag_term;
...
int arnoldix_step_double(vector_double *V, vector_double *Z, vector_double w, complex_double **H,
  ...
  int return_val;
  return_val = arnoldi_step_double(V, Z, w, H, buffer, j, prec, p, l);
  return return_val;
}

void re_scale_Uk_double(gmres_double_struct *p, Level *l, struct Thread *threading) {
  int i;
