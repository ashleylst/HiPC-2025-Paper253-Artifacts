...
void interpolation_double_alloc(Level *l) {
  ...
  l->is_double.interpolation[0] = nullptr;
...
#endif
  // ghost shell is communicated in coarse_operator_setup, so we need size=vector_size, not
  // inner_vector_size
  MALLOC_HUGEPAGES(l->is_double.op, complex_double,
                   ((size_t)OPERATOR_COMPONENT_OFFSET_double) * ((size_t)l->vector_size), 128);

  l->is_double.test_vector[0] = nullptr;
...
#endif
  FREE_HUGEPAGES(l->is_double.op, complex_double,
                 OPERATOR_COMPONENT_OFFSET_double * l->vector_size);
}
...
void define_interpolation_double_operator(complex_double **interpolation, Level *l,
  ...
  for (j = 0; j < num_eig_vect; j += offset) {
    ...
    op = l->is_double.op + j * l->vector_size + start * offset;
...
#ifdef HAVE_TM1p1
  if (g.n_flavours == 2)
    for (i = threading->n_thread * threading->core + threading->thread; i < num_aggregates;
         i += threading->n_core * threading->n_thread) {
      ...
      for (j = 0; j < num_eig_vect; j++) {
        tmp_phi1_c_re[j] = real(phi_c_pt[j]);
        tmp_phi1_c_im[j] = cimag(phi_c_pt[j]);
      }
      for (j = 0; j < num_eig_vect; j++) {
        tmp_phi2_c_re[j] = real(phi_c_pt[j + num_eig_vect]);
        tmp_phi2_c_im[j] = cimag(phi_c_pt[j + num_eig_vect]);
      }
      for (j = 0; j < num_eig_vect; j++) {
        tmp_phi1_c_re[j + OPERATOR_COMPONENT_OFFSET_double] = real(phi_c_pt[j + 2 * num_eig_vect]);
        tmp_phi1_c_im[j + OPERATOR_COMPONENT_OFFSET_double] = cimag(phi_c_pt[j + 2 * num_eig_vect]);
      }
      for (j = 0; j < num_eig_vect; j++) {
        tmp_phi2_c_re[j + OPERATOR_COMPONENT_OFFSET_double] = real(phi_c_pt[j + 3 * num_eig_vect]);
      ...
      for (j = 0; j < num_eig_vect; j += offset) {
        ...
        op = l->is_double.op + j * l->vector_size / 2 +
             i * 2 * offset * num_parent_eig_vect * aggregate_sites;
        ...
        for (k = 0; k < aggregate_sites; k++) {
          ...
          for (k1 = 0; k1 < 2; k1++) {
            for (k2 = 0; k2 < num_parent_eig_vect; k2++) {
              ...
              __m128 operator_re = _mm_load_ps((double *)op);
              __m128 operator_im = _mm_load_ps((double *)op + offset);
    ...
    for (i = threading->n_thread * threading->core + threading->thread; i < num_aggregates;
      ...
      // copy phi_c into temporary
      for (j = 0; j < num_eig_vect; j++) {
        tmp_phi_c_re[j] = real(phi_c_pt[j]);
        tmp_phi_c_im[j] = cimag(phi_c_pt[j]);
      }
      for (j = 0; j < num_eig_vect; j++) {
        tmp_phi_c_re[j + OPERATOR_COMPONENT_OFFSET_double] = real(phi_c_pt[j + num_eig_vect]);
      ...
      for (j = 0; j < num_eig_vect; j += offset) {
        phi_pt = phi + i * 2 * num_parent_eig_vect * aggregate_sites;
        op = l->is_double.op + j * l->vector_size +
             i * 2 * offset * num_parent_eig_vect * aggregate_sites;
  ...
  if (g.n_flavours == 2)
    for (i = threading->n_thread * threading->core + threading->thread; i < num_aggregates;
      ...
      for (j = 0; j < num_eig_vect; j += offset) {
        ...
        // temporary, so we can used aligned load/store, and don't have to mess around with
        // deinterleaving complex components and masking factor 2 is for low/high (refers to spin
        // components being split up for preserving gamma5-symmetry of coarse operator)
