...
static inline void pi_minus(std::complex<T>* phi, std::complex<T>* prnT, std::complex<T>* prnZ,
              std::complex<T>* prnY, std::complex<T>* prnX, int end){
  for (int i = 0; i < end/2; i += 6) {
...
void d_plus_clover_block(BlockVecPerSite<T> *bout, BlockVecPerSite<T> *bv, operator_struct<T> *op,
  ...
  pi_minus(phi, prnT, prnZ, prnY, prnX, end*blen);

  ///send prn to mu-1 direction, receive corresponding prn from mu+1 neighbor
  ghost_sendrecv_full(prnT, _T, -1, &(op->c), l, blen);
  ghost_sendrecv_full(prnZ, _Z, -1, &(op->c), l, blen);
  ghost_sendrecv_full(prnY, _Y, -1, &(op->c), l, blen);
  ghost_sendrecv_full(prnX, _X, -1, &(op->c), l, blen);

  pi_plus_kron_UH_psi(phi, op->D, prpT, prpZ, prpY, prpX, neighbor, end, blen);

  /// send prp to mu+1 neighbour, receive corresponding prp from mu-1 neighbor
  ghost_sendrecv_full(prpT, _T, +1, &(op->c), l, blen);
  ghost_sendrecv_full(prpZ, _Z, +1, &(op->c), l, blen);
  ghost_sendrecv_full(prpY, _Y, +1, &(op->c), l, blen);
  ghost_sendrecv_full(prpX, _X, +1, &(op->c), l, blen);

  /// wait for communication of prn
  ghost_wait_full(prnT, _T, -1, &(op->c), l, blen);
  ghost_wait_full(prnZ, _Z, -1, &(op->c), l, blen);
  ghost_wait_full(prnY, _Y, -1, &(op->c), l, blen);
  ghost_wait_full(prnX, _X, -1, &(op->c), l, blen);

  apply_U_psi_plus_mu(eta, op->D, prnT, prnZ, prnY, prnX, neighbor, end, blen);

  /// wait for communication of prp
  ghost_wait_full(prpT, _T, +1, &(op->c), l, blen);
  ghost_wait_full(prpZ, _Z, +1, &(op->c), l, blen);
  ghost_wait_full(prpY, _Y, +1, &(op->c), l, blen);
  ghost_wait_full(prpX, _X, +1, &(op->c), l, blen);

  lift_pi_plus_dir(eta, prpT, prpZ, prpY, prpX, end*blen);
