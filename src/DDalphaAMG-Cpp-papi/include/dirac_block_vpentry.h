...
static inline void pi_plus_per_entry(std::complex<T>* phi, std::complex<T>* D, std::complex<T>* prpT,
                                     ...
                                     const int* neighbor, int end, const int blen){
  ...
  for (int i = 0; i < end/12; i++) {
    ...
    mvmh_block(&prpZ[blen*6*neighbor[4*i+1] + 3*blen], &D[36*i+9], &pbuf[3*blen], blen);

    /// Y dir
    prn_Y_block(pbuf, &phi[i*blen*12], blen);
    mvmh_block(&prpY[blen*6*neighbor[4*i+2]], &D[36*i+18], pbuf, blen);
    mvmh_block(&prpY[blen*6*neighbor[4*i+2] + 3*blen], &D[36*i+18], &pbuf[3*blen], blen);
...
static inline void apply_U_per_entry(std::complex<T>* eta, std::complex<T>* D, std::complex<T>* prnT,
  ...
  for (int i = 0; i < end/12; i++) {
    /// T dir
    mvm_block(pbuf, &D[36*i], &prnT[blen*6*neighbor[4*i]], blen); /// apply U_mu(x) to prn, first half of x
    mvm_block(&pbuf[3*blen], &D[36*i], &prnT[blen*6*neighbor[4*i] + 3*blen], blen); /// second half of x
    ...
    mvm_block(&pbuf[3*blen], &D[36*i+9], &prnZ[blen*6*neighbor[4*i+1] + 3*blen], blen);
    pbp_su3_Z_block(pbuf, &eta[i*blen*12], blen);

    /// Y dir
    mvm_block(pbuf, &D[36*i+18], &prnY[blen*6*neighbor[4*i+2]], blen);
    mvm_block(&pbuf[3*blen], &D[36*i+18], &prnY[blen*6*neighbor[4*i+2] + 3*blen], blen);
    pbp_su3_Y_block(pbuf, &eta[i*blen*12], blen);

    /// X dir
    mvm_block(pbuf, &D[36*i+27], &prnX[blen*6*neighbor[4*i+3]], blen);
    mvm_block(&pbuf[3*blen], &D[36*i+27], &prnX[blen*6*neighbor[4*i+3] + 3*blen], blen);
...
static inline void lift_pi_plus_per_entry(std::complex<T>* eta, std::complex<T>* prpT, std::complex<T>* prpZ,
                                          std::complex<T>* prpY, std::complex<T>* prpX, int end,
                                          const int blen){
  for (int i = 0; i < end/12; i++) {
...
void ghost_sendrecv_per_entry(std::complex<T> *phi, const int mu, const int dir, comm_struct<T> *c,
  ...
  if (l->global_splitting[mu] > 1) {
    ...
    if (dir == 1) {
      ...
      int num_boundary_sites = c->num_boundary_sites[2 * mu];
...
void d_plus_clover_block(BlockVecPerEntry<T> *bout, BlockVecPerEntry<T> *bv, operator_struct<T> *op,
  ...
  pi_minus_per_entry(phi, prnT, prnZ, prnY, prnX, end, blen);

  ///send prn to mu-1 direction, receive corresponding prn from mu+1 neighbor
  ghost_sendrecv_per_entry(prnT, _T, -1, &(op->c), l, blen);
  ghost_sendrecv_per_entry(prnZ, _Z, -1, &(op->c), l, blen);
  ghost_sendrecv_per_entry(prnY, _Y, -1, &(op->c), l, blen);
  ghost_sendrecv_per_entry(prnX, _X, -1, &(op->c), l, blen);

  pi_plus_per_entry(phi, op->D, prpT, prpZ, prpY, prpX, neighbor, end, blen);

  /// send prp to mu+1 neighbour, receive corresponding prp from mu-1 neighbor
  ghost_sendrecv_per_entry(prpT, _T, +1, &(op->c), l, blen);
  ghost_sendrecv_per_entry(prpZ, _Z, +1, &(op->c), l, blen);
  ghost_sendrecv_per_entry(prpY, _Y, +1, &(op->c), l, blen);
  ghost_sendrecv_per_entry(prpX, _X, +1, &(op->c), l, blen);

  /// wait for communication of prn
  ghost_wait_per_entry(prnT, _T, -1, &(op->c), l, blen);
  ghost_wait_per_entry(prnZ, _Z, -1, &(op->c), l, blen);
  ghost_wait_per_entry(prnY, _Y, -1, &(op->c), l, blen);
  ghost_wait_per_entry(prnX, _X, -1, &(op->c), l, blen);

  apply_U_per_entry(eta, op->D, prnT, prnZ, prnY, prnX, neighbor, end, blen);

  /// wait for communication of prp
  ghost_wait_per_entry(prpT, _T, +1, &(op->c), l, blen);
  ghost_wait_per_entry(prpZ, _Z, +1, &(op->c), l, blen);
  ghost_wait_per_entry(prpY, _Y, +1, &(op->c), l, blen);
  ghost_wait_per_entry(prpX, _X, +1, &(op->c), l, blen);
