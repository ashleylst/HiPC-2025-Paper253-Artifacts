...
typedef std::complex<double> *config_double;
typedef std::complex<double> *vector_double;

template<typename T>
struct comm_struct{
  int length[8], *boundary_table[8], max_length[4], comm_start[8], in_use[8], offset, comm,
      num_even_boundary_sites[8], num_odd_boundary_sites[8], num_boundary_sites[8];
  std::complex<T>* buffer[8];
  MPI_Request sreqs[8], rreqs[8];
};

typedef struct {
  int ilde, dist_local_lattice[4], dist_inner_lattice_sites, *permutation, *gather_list,
      gather_list_length;
...
typedef struct {
  ...
  int oe_offset, self_coupling, num_even_sites, num_odd_sites, *index_table, *neighbor_table,
      *translation_table, table_dim[4], *backward_neighbor_table, table_mod_dim[4],
      *config_boundary_table[4], index_table_range;
  vector_double *buffer; //prnT, prnZ, prnY, prnX, prpT, prpZ, prpY, prpX;
  comm_struct<double> c;
...
struct operator_struct{
  ...
  T m0;
  T *D_vectorized;
  T *D_transformed_vectorized;
  T *clover_vectorized;
  T *clover_oo_inv_vectorized;
  std::complex<T>* D, *clover, *clover_oo_inv;
  std::complex<T>* odd_proj; // identity on the odd sites
  std::complex<T>* *buffer;
  comm_struct<T> c;
#ifdef HAVE_TM
  T mu, mu_odd_shift, mu_even_shift;
  std::complex<T>* tm_term;
#endif
#ifdef HAVE_TM1p1
  T epsbar, epsbar_ig5_odd_shift, epsbar_ig5_even_shift;
  std::complex<T>* epsbar_term, clover_doublet_oo_inv;
  T *clover_doublet_vectorized;
  T *clover_doublet_oo_inv_vectorized;
#endif
};
...
#ifdef GCRODR
typedef struct {
  int i, k, CU_usable, syst_size, finish, orth_against_Ck, update_CU, recompute_DPCk_poly,
      recompute_DPCk_plain, upd_ctr;
...
typedef struct {
  ...
  double tol;
  int num_restart, restart_length, timing, print, kind, initial_guess_zero, layout, v_start, v_end;
...
typedef struct {
  ...
  complex_double **H, *y, *gamma, *c, *s;
  config_double *D, *clover;
  operator_struct<double> *op;
  double tol;
  int num_restart, restart_length, timing, print, kind, initial_guess_zero, layout, v_start, v_end;
  long int total_storage;
  void (*preconditioner)(vector_double, vector_double, vector_double, const int, Level *);
  void (*eval_operator)(vector_double, vector_double, operator_struct<double> *, Level *);
...
typedef struct {
  ...
  int block_oe_offset, *index[4], dir_length[4], num_blocks, num_colors, dir_length_even[4],
      dir_length_odd[4], *oe_index[4], num_block_even_sites, num_block_odd_sites, num_aggregates,
      block_vector_size, num_block_sites, block_boundary_length[9], **block_list,
      *block_list_length;
  block_struct *block;
} schwarz_double_struct;

typedef struct {
  int num_agg, *agg_index[4], agg_length[4], *agg_boundary_index[4], *agg_boundary_neighbor[4],
      agg_boundary_length[4], num_bootstrap_vect;
  vector_double *test_vector, *interpolation, *bootstrap_vector, tmp;
  complex_double *op, *eigenvalues, *bootstrap_eigenvalues;
...
#define PROF_double_START_UNTHREADED(TYPE)                                                         \
  do {                                                                                             \
    l->prof_double.time[TYPE] -= MPI_Wtime();                                                      \
  } while (0)
...
#define PROF_double_STOP_UNTHREADED(TYPE, COUNT)                                                   \
  do {                                                                                             \
    l->prof_double.time[TYPE] += MPI_Wtime();                                                      \
    l->prof_double.count[TYPE] += COUNT;                                                           \
  } while (0)
...
#define PROF_double_START(...)                                                                     \
  GET_MACRO2(__VA_ARGS__, PROF_double_START_THREADED, PROF_double_START_UNTHREADED, padding)       \
  (__VA_ARGS__)
#define PROF_double_STOP(...)                                                                      \
  GET_MACRO3(__VA_ARGS__, PROF_double_STOP_THREADED, PROF_double_STOP_UNTHREADED, padding)         \
  (__VA_ARGS__)
