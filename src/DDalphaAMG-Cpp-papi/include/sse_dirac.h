...
void sse_add_diagonal_clover_doublet_double(double *out, complex_double *diag);
void sse_clover_double(vector_double eta, vector_double phi, operator_double_struct *op, int start,
                       int end, Level *l, struct Thread *threading);
void sse_clover_double(vector_double eta, vector_double phi, operator_double_struct *op, int start,
                       int end, Level *l, struct Thread *threading);
...
static inline void sse_mvm_double_simd_length(const complex_double *eta, const complex_double *D,
...
static inline void sse_twospin_double(complex_double *out_spin0and1, complex_double *out_spin2and3,
                                      const complex_double *in, int elements, int mu, double sign) {
}
...
static inline void sse_twospin_double(complex_double *out_spin0and1, complex_double *out_spin2and3,
  ...
  for (int spin = 0; spin < 4; spin++) {
    if (spin == 2)
      out = out_spin0and1;
    scale_re = _mm_set1_ps(sign * real(gamma_val[mu][spin]));
    scale_im = _mm_set1_ps(sign * cimag(gamma_val[mu][spin]));
    // factors of 2 are for complex
    for (int j = 0; j < 3; j++) {
      for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
        __m128 in_re =
            _mm_load_ps((double *)in + i + (2 * (3 * gamma_co[mu][spin] + j) + 0) * elements);
        __m128 in_im =
            _mm_load_ps((double *)in + i + (2 * (3 * gamma_co[mu][spin] + j) + 1) * elements);
...
static inline void sse_twospin2_p_double_simd_length(complex_double *out_spin0and1,
                                                     complex_double *out_spin2and3,
                                                     const complex_double *in, int mu) {}
// mu is according to the enum for T,Z,Y,X defined in clifford.h
static inline void sse_twospin2_p_double_simd_length(complex_double *out_spin0and1,
                                                     complex_double *out_spin2and3,
                                                     const complex_double *in, int mu) {
  ...
  for (int spin = 0; spin < 4; spin++) {
    if (spin == 2)
      out = out_spin0and1;
    scale_re = _mm_set1_ps(-real(gamma_val[mu][spin]));
...
static inline void sse_twospin2_p_double(complex_double *out_spin0and1,
                                         complex_double *out_spin2and3, const complex_double *in,
                                         int elements, int mu) {}
// mu is according to the enum for T,Z,Y,X defined in clifford.h
static inline void sse_twospin2_p_double(complex_double *out_spin0and1,
                                         complex_double *out_spin2and3, const complex_double *in,
...
static inline void sse_spin0and1_site_clover_double(const complex_double *eta,
                                                    const complex_double *phi,
                                                    const config_double clover, double shift,
                                                    int elements) {}
...
static inline void sse_spin0and1_site_clover_double(const complex_double *eta,
  ...
  if (g.csw == 0.0) {
    ...
    for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
      for (int j = 0; j < 6; j++) {
        __m128 clover_re = _mm_set1_ps(real(clover[j]));
    ...
    for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
      ...
      for (int io = 0; io < 5; io++) {
        ...
        for (int ii = io + 1; ii <= 5; ii++) {
          clover_re = _mm_set1_ps(real(clover[ic]));
...
static inline void sse_diagonal_aggregate_double(const complex_double *eta1,
                                                 const complex_double *eta2,
                                                 const complex_double *phi,
                                                 const config_double diag, int elements) {}
...
static inline void sse_diagonal_aggregate_double(const complex_double *eta1,
  ...
  for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
    __m128 zero = _mm_setzero_ps();
    for (int j = 0; j < 6; j++) {
      __m128 factor = _mm_set1_ps(real(diag[j]));
      __m128 in_re = _mm_load_ps((double *)phi + i + (2 * j + 0) * elements);
      __m128 in_im = _mm_load_ps((double *)phi + i + (2 * j + 1) * elements);
...
static inline void sse_spin2and3_site_clover_double(const complex_double *eta,
                                                    const complex_double *phi,
                                                    const config_double clover, double shift,
                                                    int elements) {}
