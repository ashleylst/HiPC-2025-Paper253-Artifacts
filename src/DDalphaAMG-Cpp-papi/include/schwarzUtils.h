...
void Schwarz::localMinres(vector_double phi, vector_double eta, vector_double latest_iter,
  ...
  int minresEnd = (g.odd_even && level->depth == 0)
  ...
  vector_double_define(lphi, 0, minresStart, minresEnd, level);

  for (int i = 0; i < level->block_iter; i++) {
    // Dr = blockD*r
    blockOperator(Dr, r, minresStart, this, level, no_threading);
    // alpha = <Dr,r>/<Dr,Dr>
    alpha = local_xy_over_xx_double(Dr, r, minresStart, minresEnd, level);
    // phi += alpha * r
    vector_double_saxpy(lphi, lphi, r, alpha, minresStart, minresEnd, level);
    // r -= alpha * Dr
    vector_double_saxpy(r, r, Dr, -alpha, minresStart, minresEnd, level);
  }
...
void Schwarz::defineLayout() {
  ...
  int a0, b0, c0, d0, a1, b1, c1, d1, block_split[4], block_size[4], agg_split[4], i, j, k, mu,
      ...
      *dt = op.table_dim, *dt_mod = op.table_mod_dim, *it = op.index_table, *count[4], flag;
  ...
  // Define coloring
  if (g.method == _ADDITIVE) // Additive
    number_of_colors = 1;
  else if (g.method == _TWO_COLOR) // Red-Black
    number_of_colors = 2;
  else if (g.method == _SIXTEEN_COLOR) { // 16 color
  ...
  for (mu = 0; mu < 4; mu++) {
    ...
    block_odd_even_offset += ((level->local_lattice[mu] / level->block_lattice[mu]) *
                              (g.my_coords[mu] / level->comm_offset[mu])) %
                             2;
    ls[mu] = 0;
  ...
  for (mu = 0; mu < 4; mu++) {
    if (level->global_splitting[mu] > 1) {
      MPI_Irecv(rbuf[mu], buf_length[mu], MPI_COMPLEX_double, level->neighbor_rank[2 * mu + 1],
                2 * mu + 1, g.comm_cart, &(op.c.rreqs[2 * mu + 1]));
    }
  }
  ...
  for (mu = 0; mu < 4; mu++) {
    if (level->global_splitting[mu] > 1) {
      ...
      MPI_Isend(buf[mu], buf_length[mu], MPI_COMPLEX_double, level->neighbor_rank[2 * mu],
                2 * mu + 1, g.comm_cart, &(op.c.sreqs[2 * mu + 1]));
      ls[mu] = 0;
    }
  }
...
void Schwarz::setupOddEvenOperator() {
  ...
  int cs = g.csw ? 42 : 12;
#else
  double *clover_pt = g.csw ? op.clover_doublet_vectorized : (double *)op.clover,
...
void Schwarz::setupOperators(operator_double_struct *op_in) {
  ...
  int i, index, n = level->num_inner_lattice_sites, *tt = op.translation_table;
  ...
  for (i = 0; i < n; i++) {
    index = tt[i];
    odd_proj_out_pt = op.odd_proj + 12 * index;
    FOR12(*odd_proj_out_pt = (complex_double)*odd_proj_in_pt; odd_proj_out_pt++; odd_proj_in_pt++;);
  }

#ifdef HAVE_TM
  tm_term_double_setup((double)(g.mu_factor[level->depth] * op_in->mu),
                       (double)(g.mu_factor[level->depth] * op_in->mu_even_shift),
                       (double)(g.mu_factor[level->depth] * op_in->mu_odd_shift), &op, level,
                       no_threading);
#endif

#ifdef HAVE_TM1p1
  epsbar_term_double_setup((double)(g.epsbar_factor[level->depth] * op_in->epsbar),
                           (double)(g.epsbar_factor[level->depth] * op_in->epsbar_ig5_even_shift),
                           (double)(g.epsbar_factor[level->depth] * op_in->epsbar_ig5_odd_shift),
                           &op, level, no_threading);
#endif
