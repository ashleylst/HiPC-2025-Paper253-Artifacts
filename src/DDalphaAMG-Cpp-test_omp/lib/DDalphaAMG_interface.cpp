...
//  *   setup_threading( ... );
//  */
// void DDalphaAMG_initialize(DDalphaAMG_init *mg_init, DDalphaAMG_parameters *mg_params,
...
//
//   conf_index_fct = nullptr;
//   vector_index_fct = nullptr;
//
//   int topo_type;
//   MPI_Topo_test(mg_init->comm_cart, &topo_type);
//
//   if (mg_init->Cart_coords == nullptr)
//     g.Cart_coords = MPI_Cart_coords;
//   else
//     g.Cart_coords = mg_init->Cart_coords;
//
//   if (mg_init->Cart_rank == nullptr)
//     g.Cart_rank = MPI_Cart_rank;
//   else
//     g.Cart_rank = mg_init->Cart_rank;
//
//   if ((g.Cart_rank == MPI_Cart_rank || g.Cart_coords == MPI_Cart_coords) && topo_type !=
//   MPI_CART) {
...
//     if (g.setup_flag)
//       m0_update(g.m0, &l[omp_get_thread_num()]);
//     else if (g.conf_flag)
//       m0_update_double(g.m0, &(g.op_double), &l[omp_get_thread_num()]);
//     re_dirac++;
//   }
//
//   // double mu;
...
//     THREADED(threading[0]->n_core)
//     if (g.setup_flag)
//       tm_term_update(g.mu, &l[omp_get_thread_num()]);
//     else if (g.conf_flag)
//       tm_term_double_setup(g.mu, g.mu_even_shift, g.mu_odd_shift, &(g.op_double), &l,
//                            threading[omp_get_thread_num()]);
//     re_dirac++;
//   }
//
// #else
...
//     warning0(
//         "Parameters mu, mu_odd_shift, mu_even_shift not supported without defining HAVE_TM
//         flag.");
// #endif
//
// #ifdef HAVE_TM1p1
...
//     }
//
//   if (update_eps || mg_params->epsbar != g.epsbar ||
...
//     THREADED(threading[0]->n_core)
//     if (g.setup_flag)
//       epsbar_term_update(&l[omp_get_thread_num()]);
//     else if (g.conf_flag)
//       epsbar_term_double_setup(g.epsbar, g.epsbar_ig5_even_shift, g.epsbar_ig5_odd_shift,
//                                &(g.op_double), &l[omp_get_thread_num()]);
//     re_dirac++;
//   }
//
// #else
//   if (mg_params->epsbar != 0 || mg_params->epsbar_ig5_odd_shift != 0 ||
//       mg_params->epsbar_ig5_even_shift != 0)
//     warning0("Parameters epsbar, epsbar_odd_shift, epsbar_even_shift not supported without "
//              "defining HAVE_TM1p1 flag.");
// #endif
//
...
//     if (g.mixed_precision)
//       THREADED(threading[0]->n_core)
//     re_setup_double(&l[omp_get_thread_num()]);
//     else THREADED(threading[0]->n_core) re_setup_double(&l[omp_get_thread_num()]);
//
//   } else if (re_dirac && g.setup_flag) { // update just the oddeven and vecorized operators
//     THREADED(threading[0]->n_core)
//     finalize_operator_update(&l[omp_get_thread_num()]);
//   }
//
//   DDalphaAMG_get_parameters(mg_params);
//
...
//   mg_status->info = g.mu;
// #else
//   warning0("DDalphaAMG_change_mu_sign called without the flag HAVE_TM enabled. Doing
//   nothing.\n"); mg_status->info = 0;
// #endif
//
//   t1 = MPI_Wtime();
...
//   complex_double phase[4] = {std::complex<double>(0,0), std::complex<double>(0,0),
...
//   if (g.print > 0)
//     printf0("%s\n", CLIFFORD_BASIS);
//   if (g.bc == _ANTIPERIODIC)
//     printf0("antiperiodic in time");
//   else if (g.bc == _TWISTED)
//     printf0("twisted (%.2f, %.2f, %.2f, %.2f)", g.twisted_bc[0], g.twisted_bc[1],
//     g.twisted_bc[2],
//             g.twisted_bc[3]);
//   else
...
//
//   if (g.bc == _ANTIPERIODIC && onb[_T]) {
//     phase[_Z] = 1;
//     phase[_Y] = 1;
//     phase[_X] = 1;
//     for (t = 1, i = 0, k = 0; t < ll[_T] + 1; t++) {
//       if (t < ll[_T])
//         phase[_T] = 1;
//       else
//         phase[_T] = -1;
...
//             }
//     }
//   } else if (g.bc == _TWISTED && (onb[_T] || onb[_Z] || onb[_Y] || onb[_X]))
//     for (t = 1, i = 0, k = 0; t < ll[_T] + 1; t++) {
//       if (!onb[_T] || t < ll[_T] || g.twisted_bc[_T] == 0)
//         phase[_T] = 1;
//       else
//         phase[_T] = exp(I * g.twisted_bc[_T]);
//       for (z = 1; z < ll[_Z] + 1; z++) {
//         if (!onb[_Z] || z < ll[_Z] || g.twisted_bc[_Z] == 0)
//           phase[_Z] = 1;
//         else
//           phase[_Z] = exp(I * g.twisted_bc[_Z]);
//         for (y = 1; y < ll[_Y] + 1; y++) {
//           if (!onb[_Y] || y < ll[_Y] || g.twisted_bc[_Y] == 0)
//             phase[_Y] = 1;
//           else
//             phase[_Y] = exp(I * g.twisted_bc[_Y]);
//           for (x = 1; x < ll[_X] + 1; x++) {
//             if (!onb[_X] || x < ll[_X] || g.twisted_bc[_X] == 0)
//               phase[_X] = 1;
//             else
//               phase[_X] = exp(I * g.twisted_bc[_X]);
//             for (mu = 0; mu < 4; mu++) {
//               if (conf_index_fct != nullptr)
//                 k = conf_index_fct(t - 1, z - 1, y - 1, x - 1, mu);
//               for (j = 0; j < 9; j++, i++, k += 2) {
...
//     THREADED(threading[0]->n_core)
//     if (g.mixed_precision)
//       operator_updates_double(&l[omp_get_thread_num()]);
//     else
//       operator_updates_double(&l[omp_get_thread_num()]);
//
...
//       method_free(&l);
//     THREADED(threading[0]->n_core) {
//       method_setup(nullptr, &l[omp_get_thread_num()]);
//       method_update(g.setup_iter[0], &l[omp_get_thread_num()]);
//     }
//     g.setup_flag = 1;
//
...
//
//     THREADED(threading[0]->n_core)
//     method_update(iterations, &l[omp_get_thread_num()]);
...
//   THREADED(threading[0]->n_core)
//   if (g.method == -1) {
//     //     cgn_double(&(g.p), &l[omp_get_thread_num()]);
//   } else if (g.mixed_precision == 2) {
//     //       fgmres_MP( &(g.p_MP), &l[omp_get_thread_num()] );
//   } else {
//     fgmres_double(&(g.p), &l[omp_get_thread_num()]);
//   }
// }
//
// static inline void correct_guess(vector_double guess, vector_double solution,
//                                  vector_double solution2, double even_dshift, double odd_dshift)
//                                  {
...
//   }
// #else
//   warning0("change_epsbar_shift_sign called without the flag HAVE_TM1p1 enabled. Doing
//   nothing.\n");
// #endif
// }
//
...
// //  |r2|/|y| < (tol - |r1|)/|Dd|*|b|/|y|
//
// static inline void DDalphaAMG_driver(double *vector1_out, double *vector1_in, double
// *vector2_out,
//                                      double *vector2_in, double tol, DDalphaAMG_status
//                                      *mg_status, int _TYPE) {
//
//   int t, z, y, x, i, j, k, mu, *ll = l.local_lattice, *gl = l.global_lattice, sl[4],
//                                precision_changed;
//   complex_double twisted_bc, tmp1, tmp2;
//   //   double phase[4] = {std::complex<double>(0,0), std::complex<double>(0,0),
//   std::complex<double>(0,0),
//   //                      std::complex<double>(0,0)},  //WHY?
//   double phase[4] = {0, 0, 0, 0}, vmin = 1, vmax = EPS_double, vtmp, nrhs, nrhs2;
...
//   mg_status->info = 0;
//
//   ASSERT(vector1_out != nullptr);
//   ASSERT(vector1_in != nullptr);
// #ifdef HAVE_TM1p1
//   if (g.n_flavours == 2) {
//     ASSERT(vector2_out != nullptr);
//     ASSERT(vector2_in != nullptr);
//   }
// #endif
//
...
//
//   for (t = 0, j = 0; t < ll[_T]; t++) {
//     if (g.bc == _TWISTED)
//       phase[_T] = g.twisted_bc[_T] * ((double)sl[_T] + t) / (double)gl[_T];
//     for (z = 0; z < ll[_Z]; z++) {
//       if (g.bc == _TWISTED)
//         phase[_Z] = phase[_T] + g.twisted_bc[_Z] * ((double)sl[_Z] + z) / (double)gl[_Z];
//       for (y = 0; y < ll[_Y]; y++) {
//         if (g.bc == _TWISTED)
//           phase[_Y] = phase[_Z] + g.twisted_bc[_Y] * ((double)sl[_Y] + y) / (double)gl[_Y];
//         for (x = 0; x < ll[_X]; x++) {
//           if (g.bc == _TWISTED) {
//             phase[_X] = phase[_Y] + g.twisted_bc[_X] * ((double)sl[_X] + x) / (double)gl[_X];
//             twisted_bc = exp(I * phase[_X]);
//           } else
//             twisted_bc = 1.;
//           if (vector_index_fct != nullptr)
...
//               for (k = 0; k < 3; k++, j++) {
// #ifndef BASIS4
//                 rhs[j] = ((complex_double)vector1_in[i + 2 * (k + 3 * mu)] +
//                           I * (complex_double)vector1_in[i + 2 * (k + 3 * mu) + 1]) *
//                          twisted_bc;
//                 rhs[j + 6] = ((complex_double)vector2_in[i + 2 * (k + 3 * mu)] +
//                               I * (complex_double)vector2_in[i + 2 * (k + 3 * mu) + 1]) *
//                              twisted_bc;
//
// #else
//                 rhs[j] = ((complex_double)vector1_in[i + 2 * (k + 3 * (3 - mu))] +
//                           I * (complex_double)vector1_in[i + 2 * (k + 3 * (3 - mu)) + 1]) *
//                          twisted_bc;
//                 rhs[j + 6] = ((complex_double)vector2_in[i + 2 * (k + 3 * (3 - mu))] +
//                               I * (complex_double)vector2_in[i + 2 * (k + 3 * (3 - mu)) + 1]) *
//                              twisted_bc;
// #endif
//
//                 if (p->initial_guess_zero == 0) {
// #ifndef BASIS4
//                   sol[j] = ((complex_double)vector1_out[i + 2 * (k + 3 * mu)] +
//                             I * (complex_double)vector1_out[i + 2 * (k + 3 * mu) + 1]) *
//                            twisted_bc;
//                   sol[j + 6] = ((complex_double)vector2_out[i + 2 * (k + 3 * mu)] +
//                                 I * (complex_double)vector2_out[i + 2 * (k + 3 * mu) + 1]) *
//                                twisted_bc;
//
// #else
//                   sol[j] = ((complex_double)vector1_out[i + 2 * (k + 3 * (3 - mu))] +
//                             I * (complex_double)vector1_out[i + 2 * (k + 3 * (3 - mu)) + 1]) *
//                            twisted_bc;
//                   sol[j + 6] = ((complex_double)vector2_out[i + 2 * (k + 3 * (3 - mu))] +
//                                 I * (complex_double)vector2_out[i + 2 * (k + 3 * (3 - mu)) + 1])
//                                 *
//                                twisted_bc;
// #endif
//                 }
...
//       tau1_gamma5_double(rhs, rhs, &l[omp_get_thread_num()]);
//     else
// #endif
//       // sol = (D_d^{-1})*g5*(D_u^{-1})*g5*rhs
//       gamma5_double(rhs, rhs, &l[omp_get_thread_num()]);
//
//     // read NOTE RESIDUAL
//     THREADED(threading[0]->n_core)
//     nrhs = global_norm_double(rhs, p->v_start, p->v_end, &l[omp_get_thread_num()]);
//     p->tol = tol / 2.;
//     solver();
//
//     THREADED(threading[0]->n_core)
// #ifdef HAVE_TM1p1
//     if (g.n_flavours == 2)
//       tau1_gamma5_double(rhs, sol, &l[omp_get_thread_num()]);
//     else
// #endif
//       gamma5_double(rhs, sol, &l[omp_get_thread_num()]);
//
// #ifdef HAVE_TM1p1
//     if (g.n_flavours == 2)
...
//     // read NOTE RESIDUAL
//     THREADED(threading[0]->n_core)
//     nrhs2 = global_norm_double(rhs, p->v_start, p->v_end, &l[omp_get_thread_num()]);
...
//     if (g.n_flavours == 2)
//       // sol = (D_h^{-1})*g5*tau1*(D_h^{-1})*g5*tau1*rhs
//       tau1_gamma5_set_even_to_zero_double(rhs, rhs, &l[omp_get_thread_num()]);
//     else
// #endif
//       // sol = (D_d^{-1})*g5*(D_u^{-1})*g5*rhs
//       gamma5_set_even_to_zero_double(rhs, rhs, &l[omp_get_thread_num()]);
//
//     // read NOTE RESIDUAL
//     THREADED(threading[0]->n_core)
//     nrhs = global_norm_double(rhs, p->v_start, p->v_end, &l[omp_get_thread_num()]);
...
// #ifdef HAVE_TM1p1
//     if (g.n_flavours == 2)
//       tau1_gamma5_set_even_to_zero_double(rhs, sol, &l[omp_get_thread_num()]);
//     else
// #endif
//       gamma5_set_even_to_zero_double(rhs, sol, &l[omp_get_thread_num()]);
//
// #ifdef HAVE_TM1p1
//     if (g.n_flavours == 2)
//       change_epsbar_shift_sign();
...
//     if (g.n_flavours == 2)
//       // sol = (D_h^{-1})*g5*tau1*(D_h^{-1})*g5*tau1*rhs
//       tau1_gamma5_set_odd_to_zero_double(rhs, rhs, &l[omp_get_thread_num()]);
//     else
// #endif
//       // sol = (D_d^{-1})*g5*(D_u^{-1})*g5*rhs
//       gamma5_set_odd_to_zero_double(rhs, rhs, &l[omp_get_thread_num()]);
//
//     // read NOTE RESIDUAL
//     THREADED(threading[0]->n_core)
//     nrhs = global_norm_double(rhs, p->v_start, p->v_end, &l[omp_get_thread_num()]);
...
// #ifdef HAVE_TM1p1
//     if (g.n_flavours == 2)
//       tau1_gamma5_set_odd_to_zero_double(rhs, sol, &l[omp_get_thread_num()]);
//     else
// #endif
//       gamma5_set_odd_to_zero_double(rhs, sol, &l[omp_get_thread_num()]);
//
// #ifdef HAVE_TM1p1
//     if (g.n_flavours == 2)
//       change_epsbar_shift_sign();
...
//   case _PRECOND:
//     THREADED(threading[0]->n_core)
//     preconditioner(sol, nullptr, rhs, _NO_RES, &l[omp_get_thread_num()]);
//     break;
//
//   case _OPERATOR:
//     THREADED(threading[0]->n_core)
//     if (g.mixed_precision == 2) {
//       apply_operator_double(sol, rhs, &(g.p_MP.dp), &l[omp_get_thread_num()]);
//     } else {
//       apply_operator_double(sol, rhs, &(g.p), &l[omp_get_thread_num()]);
//     }
//     break;
//
//   default:
...
// #ifndef BASIS4
//                 vector1_out[i + 2 * (k + 3 * mu)] = real(tmp1);
//                 vector1_out[i + 2 * (k + 3 * mu) + 1] = imag(tmp1);
//                 vector2_out[i + 2 * (k + 3 * mu)] = real(tmp2);
//                 vector2_out[i + 2 * (k + 3 * mu) + 1] = imag(tmp2);
// #else
//                   vector1_out[i + 2 * (k + 3 * (3 - mu))] = real(tmp1);
//                   vector1_out[i + 2 * (k + 3 * (3 - mu)) + 1] = imag(tmp1);
//                   vector2_out[i + 2 * (k + 3 * (3 - mu))] = real(tmp2);
//                   vector2_out[i + 2 * (k + 3 * (3 - mu)) + 1] = imag(tmp2);
// #endif
//               }
//               if (mu % 2)
//                 j += 6;
...
//   int t, z, y, x, i, j, k, n, mu, *ll = l.local_lattice, *gl = l.global_lattice, sl[4],
//                                   precision_changed;
//   complex_double twisted_bc, tmp1, tmp2;
//   double phase[4] = {0, 0, 0, 0}, vmin = 1, vmax = EPS_double, vtmp, nrhs, nrhs2;
//   gmres_double_struct *p = g.mixed_precision == 2 ? &(g.p_MP.dp) : &(g.p);
...
//   vector_double source = nullptr, solution = nullptr, solution2 = nullptr;
//   DDalphaAMG_status tmp_status;
//
//   double t0, t1;
//   t0 = MPI_Wtime();
...
//
//   if (n_shifts > 0) {
//     ASSERT(even_shifts != nullptr);
//     ASSERT(odd_shifts != nullptr);
//   }
//   if (n_shifts > 1) {
...
//
//       if (n)
//         correct_guess(sol, solution, solution2, even_shifts[n] - even_shifts[n - 1],
//                       odd_shifts[n] - odd_shifts[n - 1]);
//       // read NOTE RESIDUAL
//       THREADED(threading[0]->n_core)
//       nrhs = global_norm_double(rhs, p->v_start, p->v_end, &l[omp_get_thread_num()]);
//       p->tol = tol[n] / 2.;
...
//                   tmp2 = sol[j + 6] * twisted_bc;
// #ifndef BASIS4
//                   vector1_out[n][i + 2 * (k + 3 * mu)] = real(tmp1);
//                   vector1_out[n][i + 2 * (k + 3 * mu) + 1] = imag(tmp1);
//                   vector2_out[n][i + 2 * (k + 3 * mu)] = real(tmp2);
//                   vector2_out[n][i + 2 * (k + 3 * mu) + 1] = imag(tmp2);
// #else
//                       vector1_out[n][i + 2 * (k + 3 * (3 - mu))] = real(tmp1);
//                       vector1_out[n][i + 2 * (k + 3 * (3 - mu)) + 1] = imag(tmp1);
//                       vector2_out[n][i + 2 * (k + 3 * (3 - mu))] = real(tmp2);
//                       vector2_out[n][i + 2 * (k + 3 * (3 - mu)) + 1] = imag(tmp2);
// #endif
//                 }
//                 if (mu % 2)
//                   j += 6;
...
// }
//
// static inline void DDalphaAMG_proj_driver(double *vector_out, double *vector_in, int level,
//                                           DDalphaAMG_status *mg_status, int _TYPE) {
//
//   int t, z, y, x, i, j, mu, *ll;
//
//   Level *ltmp = &l, *from, *to;
//   for (i = 0; i < level; i++)
...
//   ASSERT(vector_out != nullptr);
//   ASSERT(vector_in != nullptr);
//
...
//             rhs[j] = ((complex_double)vector_in[i + 2 * mu] +
//                       I * (complex_double)vector_in[i + 2 * mu + 1]);
//         }
//
//   switch (_TYPE) {
//
//   case _RESTRICT:
//     THREADED(threading[0]->n_core)
//     restrict_double(sol, rhs, from[omp_get_thread_num()]);
//     break;
//
//   case _PROLONGATE:
//     THREADED(threading[0]->n_core)
//     interpolate3_double(sol, rhs, to[omp_get_thread_num()]);
//     break;
//
//   case _OPERATOR:
//     THREADED(threading[0]->n_core)
//     apply_operator_double(sol, rhs, &(from->p_double), from[omp_get_thread_num()]);
//     break;
//
//   default:
//     warning0("_TYPE not found in DDalphaAMG_driver. Returing vector in as vector out.");
...
//             vector_out[i + 2 * mu] = (double)real(sol[j]);
//             vector_out[i + 2 * mu + 1] = (double)imag(sol[j]);
//           }
//         }
//
//   mg_status->success = 1;
...
// #ifdef HAVE_TM1p1
//   THREADED(threading[0]->n_core)
//   data_layout_n_flavours(n, &l[omp_get_thread_num()]);
// #else
//       if (n == 2)
...
//                       DDalphaAMG_status *mg_status) {
//   DDalphaAMG_driver(vector_out, vector_in, nullptr, nullptr, tol, mg_status, _SOLVE);
// }
//
// void DDalphaAMG_solve_doublet(double *vector1_out, double *vector1_in, double *vector2_out,
//                               double *vector2_in, double tol, DDalphaAMG_status *mg_status) {
//   set_n_flavours(2);
...
//                                          double *vector2_out, double *vector2_in, double tol,
//                                          DDalphaAMG_status *mg_status) {
//   gmres_double_struct *p = g.mixed_precision == 2 ? &(g.p_MP.dp) : &(g.p);
//   set_n_flavours(2);
//   p->initial_guess_zero = 0;
...
// }
//
// void DDalphaAMG_solve_ms_doublet(double **vector1_out, double *vector1_in, double **vector2_out,
//                                  double *vector2_in, double *even_shifts, double *odd_shifts,
//                                  int n_shifts, double *tol, DDalphaAMG_status *mg_status) {
//   set_n_flavours(2);
//   DDalphaAMG_ms_driver(vector1_out, vector1_in, vector2_out, vector2_in, even_shifts, odd_shifts,
//                        n_shifts, tol, mg_status, _SOLVE);
//   set_n_flavours(1);
// }
//
// void DDalphaAMG_solve_squared(double *vector_out, double *vector_in, double tol,
//                               DDalphaAMG_status *mg_status) {
//   DDalphaAMG_driver(vector_out, vector_in, nullptr, nullptr, tol, mg_status, _SOLVE_SQ);
// }
//
// void DDalphaAMG_solve_doublet_squared(double *vector1_out, double *vector1_in, double
// *vector2_out,
//                                       double *vector2_in, double tol,
//                                       DDalphaAMG_status *mg_status) {
//   set_n_flavours(2);
...
//   set_n_flavours(2);
//   DDalphaAMG_ms_driver(vector1_out, vector1_in, vector2_out, vector2_in, even_shifts, odd_shifts,
//                        n_shifts, tol, mg_status, _SOLVE_SQ);
//   set_n_flavours(1);
// }
//
// void DDalphaAMG_solve_squared_odd(double *vector_out, double *vector_in, double tol,
//                                   DDalphaAMG_status *mg_status) {
//   DDalphaAMG_driver(vector_out, vector_in, nullptr, nullptr, tol, mg_status, _SOLVE_SQ_ODD);
// }
//
...
//                                           DDalphaAMG_status *mg_status) {
//   set_n_flavours(2);
//   DDalphaAMG_driver(vector1_out, vector1_in, vector2_out, vector2_in, tol, mg_status,
//                     _SOLVE_SQ_ODD);
//   set_n_flavours(1);
// }
//
...
//                                              double **vector2_out, double *vector2_in,
//                                              double *even_shifts, double *odd_shifts, int
//                                              n_shifts, double *tol, DDalphaAMG_status *mg_status)
//                                              {
//   set_n_flavours(2);
//   DDalphaAMG_ms_driver(vector1_out, vector1_in, vector2_out, vector2_in, even_shifts, odd_shifts,
//                        n_shifts, tol, mg_status, _SOLVE_SQ_ODD);
//   set_n_flavours(1);
// }
//
// void DDalphaAMG_solve_squared_even(double *vector_out, double *vector_in, double tol,
//                                    DDalphaAMG_status *mg_status) {
//   DDalphaAMG_driver(vector_out, vector_in, nullptr, nullptr, tol, mg_status, _SOLVE_SQ_EVEN);
// }
//
...
//                                               double **vector2_out, double *vector2_in,
//                                               double *even_shifts, double *odd_shifts, int
//                                               n_shifts, double *tol, DDalphaAMG_status
//                                               *mg_status) {
//   set_n_flavours(2);
//   DDalphaAMG_ms_driver(vector1_out, vector1_in, vector2_out, vector2_in, even_shifts, odd_shifts,
//                        n_shifts, tol, mg_status, _SOLVE_SQ_EVEN);
//   set_n_flavours(1);
// }
//
// void DDalphaAMG_apply_operator(double *vector_out, double *vector_in,
//                                DDalphaAMG_status *mg_status) {
//   DDalphaAMG_driver(vector_out, vector_in, nullptr, nullptr, 0, mg_status, _OPERATOR);
// }
//
// void DDalphaAMG_apply_operator_doublet(double *vector1_out, double *vector1_in, double
// *vector2_out,
//                                        double *vector2_in, DDalphaAMG_status *mg_status) {
//   set_n_flavours(2);
...
//                                DDalphaAMG_status *mg_status) {
//   DDalphaAMG_driver(vector_out, vector_in, nullptr, nullptr, 0, mg_status, _PRECOND);
// }
//
// void DDalphaAMG_preconditioner_doublet(double *vector1_out, double *vector1_in, double
// *vector2_out,
//                                        double *vector2_in, DDalphaAMG_status *mg_status) {
//   set_n_flavours(2);
...
//   if (vector != nullptr) {
//     int start, end;
//     compute_core_start_end(0, l.inner_vector_size, &start, &end, &l,
//                            threading[omp_get_thread_num()]);
//     vector_double_define((vector_double)vector, re + I * im, start, end, &l);
//   } else {
//     warning0("Vector nullptr when calling DDalphaAMG_define_vector_const!");
//   }
// }
//
...
//   THREADED(threading[0]->n_core)
//   if (vector != nullptr) {
//     norm = global_norm_double((vector_double)vector, 0, l.inner_vector_size, &l,
//                               threading[omp_get_thread_num()]);
//   } else {
//     warning0("Vector nullptr when calling DDalphaAMG_define_vector_const!");
//   }
...
//
//   THREADED(threading[0]->n_core)
//   if (vector_out != nullptr && x != nullptr && y != nullptr) {
//     int start, end;
//     compute_core_start_end(0, l.inner_vector_size, &start, &end, &l,
//                            threading[omp_get_thread_num()]);
//     vector_double_saxpy((vector_double)vector_out, (vector_double)x, (vector_double)y, a, start,
//                         end, &l);
//   } else {
//     warning0("Vector nullptr when calling DDalphaAMG_define_vector_const!");
//   }
...
//   printf00("\n");
//   THREADED(threading[0]->n_core)
//   test_routine(&l[omp_get_thread_num()]);
//
//   if (g.test < 1e-5)
//     mg_status->success = 1;
//   else
