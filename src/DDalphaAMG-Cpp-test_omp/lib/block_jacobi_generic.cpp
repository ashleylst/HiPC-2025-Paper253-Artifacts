...
void bj_direct_op_apply_double(vector_double out, vector_double in, Level *l) {

  int start = 0, end = l->p_double.op->num_even_sites;

  int site_size = l->num_lattice_site_var;
  int lda = SIMD_LENGTH_double * ((site_size + SIMD_LENGTH_double - 1) / SIMD_LENGTH_double);
#ifdef HAVE_TM1p1
  OPERATOR_TYPE_double *clover = (g.n_flavours == 2)
  ...
  for (int i = start; i < end; i++) {
    ...
    cgemv(site_size, clover + i * 2 * site_size * lda, lda, (double *)(in + i * site_size),
          (double *)(out + i * site_size));
  }
}

void block_jacobi_apply_double(vector_double out, vector_double in, gmres_double_struct *p,
                               Level *l) {
  if (out == in) {
    return;
  }
  ...
  printf("(proc=%d) PRINTING MATRICES! (depth=%d, nr_procs=%d) \n", g.my_rank, l->depth,
  g.num_processes);

  {
    ...
    if ( g.my_rank==0 ) gather_buff  = (vector_double) malloc(
  g.num_processes*(end-start)*sizeof(complex_double) );
    ...
    for (i=0;i<g.num_processes;i++) {
      ...
      for (int j=0;j<(end-start);j++) {
        ...
        //p->eval_operator( out, in, p->op, l );
        coarse_apply_schur_complement_double( left,right,p->op,l);

        MPI_Gather( left, end-start, MPI_COMPLEX_double, gather_buff, end-start, MPI_COMPLEX_double,
  0, g.comm_cart);

        if ( g.my_rank==0 ) {
          for ( int k=0; k<(end-start)*g.num_processes; k++ ) {
            fprintf(fp, "%f+%fj -- ", real(gather_buff[k]), cimag(gather_buff[k]));
          }
          fprintf(fp, "\n");
        }
      }
  ...
  g.num_processes*(end-start)*sizeof(complex_double) );
    ...
    for (i=0;i<g.num_processes;i++) {
      ...
      for (int j=0;j<(end-start);j++) {
        ...
        //p->eval_operator( out, in, p->op, l );
        //coarse_apply_schur_complement_double( left,right,p->op,l );
        coarse_local_apply_schur_complement_double( left,right,p->op,l );
  ...
  */
  ...
  if (p->block_jacobi_double.BJ_usable == 1) {
    local_apply_polyprec_double(out, nullptrptr, in, 0, l);
    /*
    {
      double tmpx1, tmpx2;
      int start = p->v_start;
      ...
      loc_p->eval_operator(rhs, solution, loc_p->op, l);

      // x ~ w
      local_apply_polyprec_double( x, nullptr, rhs, 0, l );
...
#ifdef BJ_DIR_SOLVS
  bj_direct_op_apply_double(out, in, l);
#else
  if (p->block_jacobi_double.BJ_usable == 1) {
    local_apply_polyprec_double(out, nullptr, in, 0, l);
  }
#endif
...
/*
void block_jacobi_restore_from_buffer_double( vector_double out, gmres_double_struct *p,
Level *l ) {
  //vector_double out_x=0;
  // TODO :
  // 		1. assign out_x to the BJ buffer (the same buffer used by
block_jacobi_apply_double(...))
