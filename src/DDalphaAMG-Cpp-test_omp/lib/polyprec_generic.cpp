...
void print_matrix_double(complex_double *A, int mv, int mh) {
  ...
  //     for(j=0; j < mh; j++)
  //     {
  //             fprintf(stdout, "%6.6f +i%6.6f\t", real(A[i*mh + j]), cimag(A[i*mh+j]));
  //     }
  //     fprintf(stdout, "\n");
  // }
  // printf("--\n");
  ...
  for (i = 0; i < mh; i++) {
    for (j = 0; j < mv; j++) {
      fprintf(stdout, "%6.6f +i%6.6f\t", real(A[j * mh + i]), cimag(A[j * mh + i]));
    }
    fprintf(stdout, "\n");
  }
  printf("--\n");
...
void print_vector_double(char *desc, vector_double w, int n) {
  ...
  printf0("\n %s\n", desc);
  for (j = 0; j < n; j++)
    printf0(" (%6.6f,%6.6f)", real(w[j]), cimag(w[j]));
  printf0("\n");
}
...
void update_lejas_double(gmres_double_struct *p, Level *l) {
  int start, end;
  ...
  fgmres_itersx = fgmres_double(p, l);
...
void re_construct_lejas_double(Level *l) { update_lejas_double(&(l->p_double), l); }

void apply_polyprec_double(vector_double phi, vector_double Dphi, vector_double eta, int res,
                           Level *l) {
  ...
  compute_core_start_end(l->p_double.v_start, l->p_double.v_end, &start, &end, l);
  ...
  for (i = 1; i < d_poly; i++) {
    ...
    g.use_pers_comms1 = 1;
#endif
    apply_operator_double(temp, product, &l->p_double, l);
#ifdef PERS_COMMS
    g.pers_comms_id2 = -1;
    g.use_pers_comms1 = 0;
#endif
