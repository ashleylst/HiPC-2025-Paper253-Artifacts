...
void var_table_insert(var_table *t, var_table_entry e) {
  ...
  if (t->entry == nullptr) {
    ...
    *(t->entry) = e;
    t->entry->next = nullptr;

  } else {
    t->iterator = t->entry;
    while (t->iterator->next != nullptr)
      t->iterator = t->iterator->next;

    MALLOC(t->iterator->next, var_table_entry, 1);
    *(t->iterator->next) = e;
    t->iterator->next->next = nullptr;
  }
}
...
void scan_var(var_table *t, Level *l) {
  ...
  if (t->iterator == nullptr) {
    error0("unable to scan variable \"%s\"\n", t->scan_var);
    //   } else {
    //     if (strcmp(t->iterator->datatype, "int") == 0)
    //       SCAN_VAR(t->iterator->pt, int, t->start_val, t->end_val, t->step_size,
    //       t->multiplicative,
    //                t->iterator->name, l);
    //     else
    //       SCAN_VAR(t->iterator->pt, double, t->start_val, t->end_val, t->step_size,
    //       t->multiplicative,
    //                t->iterator->name, l);
  }
...
void plot_table(var_table *t) {
  ...
  printf0(
      "----------+----------+----------+----------+----------+----------+----------+----------+\n");
  printf0(
      "trckd val | stp time | slv iter | slv time | crs iter | crs time | slv err  | CGNR err |\n");
  printf0(
      "----------+----------+----------+----------+----------+----------+----------+----------+\n");
