  ...
  for (i = 0; i < bins; i++) {
    for (j = 0; j < n_ind_to_swap / 2; j++)
      for (k = 0; k < fixed_size; k++) {
        ...
        data_pt[i * bin_size + j * fixed_size + k] =
            data_pt[(i + 1) * bin_size - (j + 1) * fixed_size + k];
      }
    for (j = 0; j < n_ind_to_swap / 2; j++)
      for (k = 0; k < fixed_size; k++)
...
void lime_read_info(FILE **fin, lime_fileinfo *lime, char *binary_data) {
  ...
  while (limeReaderNextRecord(limereader) != LIME_EOF) {
    ...
    if (strcmp(lime_type, "ildg-format") == 0) {
      ...
      sscanf(lime_getParam("<precision>", lime_data, lime->data_size, '<'), "%i",
             &(lime->precision));
    ...
    if (strcmp(lime_type, "etmc-source-format") == 0 ||
        strcmp(lime_type, "etmc-propagator-format") == 0) {
      lime->data_size = limeReaderBytes(limereader);
      lime_data = (char *)malloc(lime->data_size);
      limeReaderReadData((void *)lime_data, &(lime->data_size), limereader);
      sscanf(lime_getParam("<precision>", lime_data, lime->data_size, '<'), "%i",
...
void lime_check_info(lime_fileinfo lime, n_uint64_t data_size_per_site) {
  ...
  ASSERT(lime.data_size == (n_uint64_t)vol * data_size_per_site *
                               ((n_uint64_t)(lime.precision == 64 ? sizeof(complex_double)
                                                                  : sizeof(complex_double))));
}

void lime_write_info(FILE **fout, char *binary_data, n_uint64_t data_size_per_site,
                     n_uint64_t *offset) {

  LimeWriter *limewriter;
  LimeRecordHeader *limeheader = nullptr;
  n_uint64_t message_length;
  char tmp_string[STRINGLENGTH];

  ASSERT((limewriter = limeCreateWriter(*fout)) != (LimeWriter *)nullptr);
  ...
  ASSERT(limeheader != (LimeRecordHeader *)nullptr);
  ...
  sprintf(tmp_string,
          ...
          "precision>\n\t<flavours>1</flavours>\n\t<lx>%d</lx>\n\t<ly>%d</ly>\n\t<lz>%d</"
          "lz>\n\t<lt>%d</lt>\n\t<spin>4</spin>\n\t<colour>3</colour>\n</etmcFormat>",
          g.global_lattice[0][X], g.global_lattice[0][Y], g.global_lattice[0][Z],
          g.global_lattice[0][T]);
  message_length = (n_uint64_t)strlen(tmp_string);
 ...
 *********************************************************************************/
#ifdef HAVE_LIME
  int t, z, y, x, i, k, desired_rank, *gl = g.global_lattice[0], *ll = g.local_lattice[0],
                                      read_size = 4 * 18 * ll[X], precision = 64;
  ...
  if (g.my_rank == 0) {
    ...
    if (g.print > 0)
      printf0("Lattice size nx=%d ny=%d nz=%d nt=%d\n", lime.l[X], lime.l[Y], lime.l[Z], lime.l[T]);
    if (g.print > 0)
      printf0("Reading configuration in %s precision.\n", precision == 64 ? "double" : "single");
...
void lime_read_conf(double *input_data, char *input_name, double *conf_plaq, Global *global) {
 ...
 *********************************************************************************/
#ifdef HAVE_LIME
  int t, z, y, x, i, k, desired_rank, *gl = global->global_lattice[0],
                                      *ll = global->local_lattice[0], read_size = 4 * 18 * ll[X],
                                      precision = 64;
  double *input_data_pt, plaq;
  ...
  if (global->my_rank == 0) {
    ...
    precision = lime.precision;
    if (precision == 32) {
      double_buffer = new double[read_size];
    }
    if (global->print > 0)
      printf0("Lattice size nx=%d ny=%d nz=%d nt=%d\n", lime.l[X], lime.l[Y], lime.l[Z], lime.l[T]);
    if (global->print > 0)
...
void lime_read_vector(double *phi, char *filename) {
  ...
  int t, z, y, x, *gl = g.global_lattice[0], *ll = g.local_lattice[0], bar_size = 24 * ll[X],
                  desired_rank, precision = 64;
...
void lime_write_vector(double *phi, char *filename) {
  ...
  if (g.my_rank == 0) {
    ...
    n_uint64_t offset;
    lime_write_info(&file, "scidac-binary-data", (n_uint64_t)4 * 3 * sizeof(complex_double),
                    &offset);
    ...
    for (z = 0; z < gl[Z]; z++)
      for (y = 0; y < gl[Y]; y++)
        for (x = 0; x < gl[X]; x += ll[X]) {
          ...
          if (g.my_rank == 0) {
            MPI_Irecv(buffer_pt->next->data, bar_size, MPI_DOUBLE, desired_rank, 0, g.comm_cart,
                      &rreq);
            if (!(t == 0 && z == 0 && y == 0 && x == 0)) {
              for (i = 0; i < bar_size; i++) {
                byteswap8((char *)(buffer_pt->data + i));
