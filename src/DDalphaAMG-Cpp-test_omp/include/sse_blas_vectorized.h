...
static inline void sse_cgem_inverse(const int N, double *A_inverse, double *A, int lda) {
  ...
  for (j = 0; j < N; j++) {
    for (i = 0; i < N; i++) {
      A_inverse[i + 2 * j * lda] = real(tmpA_inverse[i + j * N]);
...
static inline void sse_cgemv(const int N, const OPERATOR_TYPE_double *A, int lda, const double *B,
                             double *C) {
  int i, j;
  ...
  for (i = 0; i < lda; i += SIMD_LENGTH_double) {
    ...
    C_im[i / SIMD_LENGTH_double] =
        _mm_setr_ps(C[2 * i + 1], C[2 * i + 3], C[2 * i + 5], C[2 * i + 7]);
  }
  ...
  for (j = 0; j < N; j++) {
    ...
    for (i = 0; i < lda; i += SIMD_LENGTH_double) {
      ...
      // C += A*B
      cfmadd(A_re, A_im, B_re, B_im, &(C_re[i / SIMD_LENGTH_double]),
             &(C_im[i / SIMD_LENGTH_double]));
    }
  }
...
static inline void sse_cgenmv(const int N, const OPERATOR_TYPE_double *A, int lda, const double *B,
                              double *C) {
  int i, j;
  ...
  for (j = 0; j < N; j++) {
    ...
    for (i = 0; i < lda; i += SIMD_LENGTH_double) {
      ...
      // C -= A*B
      cfnmadd(A_re, A_im, B_re, B_im, &(C_re[i / SIMD_LENGTH_double]),
              &(C_im[i / SIMD_LENGTH_double]));
    }
  }
...
static inline void sse_cgemv_padded(const int N, const OPERATOR_TYPE_double *A, int lda, int padded,
  ...
  if (g.n_flavours == 2) {
    ...
    for (i = 0; i < lda; i += SIMD_LENGTH_double) {
      ...
      C1_re[i / SIMD_LENGTH_double] =
          _mm_setr_ps(C[2 * ip], C[2 * ip + 2], C[2 * ip + 4], C[2 * ip + 6]);
      C1_im[i / SIMD_LENGTH_double] =
          _mm_setr_ps(C[2 * ip + 1], C[2 * ip + 3], C[2 * ip + 5], C[2 * ip + 7]);
      C2_re[i / SIMD_LENGTH_double] =
          _mm_setr_ps(C[2 * (ip + padded)], C[2 * (ip + padded) + 2], C[2 * (ip + padded) + 4],
                      C[2 * (ip + padded) + 6]);
      C2_im[i / SIMD_LENGTH_double] =
          _mm_setr_ps(C[2 * (ip + padded) + 1], C[2 * (ip + padded) + 3], C[2 * (ip + padded) + 5],
                      C[2 * (ip + padded) + 7]);
    }
    ...
    for (j = 0; j < N; j++) {
      ...
      for (i = 0; i < lda; i += SIMD_LENGTH_double) {
        ...
        // C += A*B
        cfmadd(A_re, A_im, B1_re, B1_im, &(C1_re[i / SIMD_LENGTH_double]),
               &(C1_im[i / SIMD_LENGTH_double]));
        cfmadd(A_re, A_im, B2_re, B2_im, &(C2_re[i / SIMD_LENGTH_double]),
               &(C2_im[i / SIMD_LENGTH_double]));
      }
    }
...
static inline void sse_cgenmv_padded(const int N, const OPERATOR_TYPE_double *A, int lda,
                                     int padded, const double *B, double *C) {
  int i, j, ip;
  ...
  if (g.n_flavours == 2) {
    ...
    for (j = 0; j < N; j++) {
      ...
      for (i = 0; i < lda; i += SIMD_LENGTH_double) {
        ...
        // C += A*B
        cfnmadd(A_re, A_im, B1_re, B1_im, &(C1_re[i / SIMD_LENGTH_double]),
                &(C1_im[i / SIMD_LENGTH_double]));
        cfnmadd(A_re, A_im, B2_re, B2_im, &(C2_re[i / SIMD_LENGTH_double]),
                &(C2_im[i / SIMD_LENGTH_double]));
      }
    }
