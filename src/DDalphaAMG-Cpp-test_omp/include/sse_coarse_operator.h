...
static inline void sse_set_coarse_self_coupling_double(complex_double *spin_0_1,
                                                       complex_double *spin_2_3, complex_double *V,
                                                       Level *l, int site, const int n_rhs,
                                                       complex_double *tmp);
static inline void sse_set_coarse_neighbor_coupling_double(complex_double *spin_0_1,
                                                           complex_double *spin_2_3,
                                                           complex_double *V, const int mu,
                                                           Level *l, int site, const int n_rhs,
                                                           complex_double *tmp);
static inline void sse_coarse_spinwise_site_self_couplings_double(complex_double *eta1,
                                                                  complex_double *eta2,
                                                                  complex_double *phi,
                                                                  config_double clover,
...
static inline void sse_coarse_spinwise_site_self_couplings_double(complex_double *eta1,
                                                                  ...
                                                                  config_double clover,
                                                                  int elements, Level *l) {}
static inline void sse_coarse_aggregate_block_diagonal_double(complex_double *eta1,
                                                              complex_double *eta2,
                                                              complex_double *phi,
                                                              config_double clover, int elements,
                                                              Level *l) {}
static inline void sse_set_coarse_block_diagonal_double(complex_double *spin_0_1,
                                                        complex_double *spin_2_3, complex_double *V,
                                                        Level *l, int site, const int n_rhs,
                                                        complex_double *tmp) {}
...
static inline void sse_set_coarse_self_coupling_double(complex_double *spin_0_1,
  ...
  for (int n = 0; n < n_rhs; n++) {
    ...
    spin_0_1_pt =
        (double *)(spin_0_1 + (n / SIMD_LENGTH_double) * 2 * offset * SIMD_LENGTH_double) +
        n % SIMD_LENGTH_double;
    spin_2_3_pt =
        (double *)(spin_2_3 + (n / SIMD_LENGTH_double) * 2 * offset * SIMD_LENGTH_double) +
        n % SIMD_LENGTH_double;
    ...
    for (k = 0; k < max; k += SIMD_LENGTH_double) {
      ...
      interpolation_data =
          (double *)(V + k * l->vector_size + fine_components * SIMD_LENGTH_double * site);
      ...
      for (m = 0; m < offset; m++) {
        ...
        __m128 interpolation_data_re =
            _mm_load_ps(interpolation_data + (2 * m + 0) * component_offset);
        __m128 interpolation_data_im =
            _mm_load_ps(interpolation_data + (2 * m + 1) * component_offset);

        cfmadd_conj(interpolation_data_re, interpolation_data_im, spin_0_1_re, spin_0_1_im,
                    &buffer_re, &buffer_im);
      }
      ...
      for (m = offset; m < 2 * offset; m++) {
        ...
        cfmadd_conj(interpolation_data_re, interpolation_data_im, spin_2_3_re, spin_2_3_im,
                    &buffer_re, &buffer_im);
      }
    ...
    for (k = 0; k < OPERATOR_COMPONENT_OFFSET_double; k += SIMD_LENGTH_double) {
      ...
      interpolation_data =
          (double *)(V + k * l->vector_size + fine_components * component_offset * site);
    ...
    for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
      for (int column = 0; column < num_eig_vect; column++) {
        ...
        for (int row = 0; row <= column; row++) {
          ...
          clover_re = _mm_set1_ps(real(clover[(column * column + column) / 2 + row]));
        ...
        for (int row = column + 1; row < num_eig_vect; row++) {
          out_re = _mm_load_ps((double *)eta[s] + i + (2 * row + 0) * elements);
          out_im = _mm_load_ps((double *)eta[s] + i + (2 * row + 1) * elements);
          clover_re = _mm_set1_ps(real(clover[(row * row + row) / 2 + column]));
  ...
  for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
    for (int column = 0; column < num_eig_vect; column++) {
      ...
      for (int row = 0; row < num_eig_vect; row++) {
        ...
        // load transposed B
        clover_re = _mm_set1_ps(real(clover[row * num_eig_vect + column]));
  ...
  for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
    for (int column = 0; column < num_eig_vect; column++) {
      ...
      for (int row = 0; row < num_eig_vect; row++) {
        ...
        clover_re = _mm_set1_ps(real(clover[column * num_eig_vect + row]));
...
static inline void sse_coarse_aggregate_block_diagonal_double(complex_double *eta1,
                                                              complex_double *eta2,
                                                              complex_double *phi,
                                                              config_double block, int elements,
                                                              Level *l) {
    ...
    for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
      for (int column = 0; column < num_eig_vect; column++) {
        ...
        for (int row = 0; row <= column; row++) {
          out_re = _mm_load_ps((double *)eta[s] + i + (2 * row + 0) * elements);
          out_im = _mm_load_ps((double *)eta[s] + i + (2 * row + 1) * elements);
          block_re = _mm_set1_ps(real(block[(column * column + column) / 2 + row]));
        ...
        for (int row = column + 1; row < num_eig_vect; row++) {
          out_re = _mm_load_ps((double *)eta[s] + i + (2 * row + 0) * elements);
          out_im = _mm_load_ps((double *)eta[s] + i + (2 * row + 1) * elements);
          block_re = _mm_set1_ps(real(block[(row * row + row) / 2 + column]));
