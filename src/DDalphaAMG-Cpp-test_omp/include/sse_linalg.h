...
static inline void sse_aggregate_gram_schmidt_double(complex_double *V, const int num_vec, Level *l,
                                                     struct Thread *threading);
// Gram-Schmidt on a block of vectors, used by Block-Gram-Schmidt
static inline void sse_aggregate_gram_schmidt_block_double(double *V, int num_vec,
...
static inline void sse_aggregate_block_dot_block_double(double *S, double *U, double *B,
                                                        int num_vec, int leading_dimension,
...
static inline void sse_aggregate_gram_schmidt_double(complex_double *V, const int num_vec, Level *l,
  ...
  for (j = threading->n_thread * threading->core + threading->thread; j < num_aggregates;
    ...
    for (k1 = 0; k1 < num_vec; k1++) {
      v_pt1 =
          (double *)V + (k1 / ldv) * V_block_offset * ldv + k1 % ldv + j * aggregate_size * 2 * ldv;
      ...
      for (i = 0; i < aggregate_size;) {
        ...
        for (k = 0; k < offset; k++, i++) {
          ...
          for (k2 = 0; k2 < OPERATOR_COMPONENT_OFFSET_double; k2 += SIMD_LENGTH_double) {
            ...
            cfmadd_conj(v1_re, v1_im, v2_re, v2_im, &alpha1_re[k2 / SIMD_LENGTH_double],
                        &alpha1_im[k2 / SIMD_LENGTH_double]);
          }
        }
        for (k = 0; k < offset; k++, i++) {
          v1_re = _mm_set1_ps(v_pt1[(2 * i + 0) * ldv]);
          ...
          for (k2 = 0; k2 < OPERATOR_COMPONENT_OFFSET_double; k2 += SIMD_LENGTH_double) {
            v2_re = _mm_load_ps(v_pt2 + (2 * i + 0) * ldv + k2 * V_block_offset);
            v2_im = _mm_load_ps(v_pt2 + (2 * i + 1) * ldv + k2 * V_block_offset);
            cfmadd_conj(v1_re, v1_im, v2_re, v2_im, &alpha2_re[k2 / SIMD_LENGTH_double],
                        &alpha2_im[k2 / SIMD_LENGTH_double]);
          }
        }
      }
      ...
      for (i = 0; i < aggregate_size;) {
        for (k = 0; k < offset; k++, i++) {
          ...
          for (k2 = (k1 / SIMD_LENGTH_double) * SIMD_LENGTH_double;
               k2 < OPERATOR_COMPONENT_OFFSET_double; k2 += SIMD_LENGTH_double) {

            v2_re = _mm_load_ps(v_pt2 + (2 * i + 0) * ldv + k2 * V_block_offset);
            v2_im = _mm_load_ps(v_pt2 + (2 * i + 1) * ldv + k2 * V_block_offset);

            if (k2 < k1 + 1) {
              double mask[SIMD_LENGTH_double]
                  __attribute__((aligned(sizeof(double) * SIMD_LENGTH_double)));
              ...
              masked_cfnmadd(alpha1_re[k2 / SIMD_LENGTH_double], alpha1_im[k2 / SIMD_LENGTH_double],
                             v1_re, v1_im, &v2_re, &v2_im, maskreg);
            } else {
              cfnmadd(alpha1_re[k2 / SIMD_LENGTH_double], alpha1_im[k2 / SIMD_LENGTH_double], v1_re,
                      v1_im, &v2_re, &v2_im);
            }
        ...
        for (k = 0; k < offset; k++, i++) {
          ...
          for (k2 = (k1 / SIMD_LENGTH_double) * SIMD_LENGTH_double;
            ...
            if (k2 < k1 + 1) {
              ...
              masked_cfnmadd(alpha2_re[k2 / SIMD_LENGTH_double], alpha2_im[k2 / SIMD_LENGTH_double],
                             v1_re, v1_im, &v2_re, &v2_im, maskreg);
            } else {
              cfnmadd(alpha2_re[k2 / SIMD_LENGTH_double], alpha2_im[k2 / SIMD_LENGTH_double], v1_re,
                      v1_im, &v2_re, &v2_im);
            }
        ...
        if (k1 + 1 < num_vec) {
          double *v_pt = (double *)V + ((k1 + 1) / ldv) * V_block_offset * ldv + (k1 + 1) % ldv +
                         j * aggregate_size * 2 * ldv;
          i -= 2 * offset;
          for (k = 0; k < offset; k++, i++) {
    ...
    for (k1 = 0; k1 < num_vec; k1++) {
      ...
      for (i = 0; i < aggregate_size;) {
        for (k = 0; k < offset; k++, i++) {
          ...
          double buffer[SIMD_LENGTH_double]
              __attribute__((aligned(sizeof(double) * SIMD_LENGTH_double)));
...
static inline void sse_aggregate_block_dot_block_double(double *S, double *U, double *B,
  ...
  for (int jp = threading->core; jp < num_aggregates; jp += threading->n_core) {
    ...
    Up = U + 2 * component * offset * leading_dimension +
         2 * 2 * j * aggregate_size * leading_dimension;
    Bp = B + 2 * component * offset * leading_dimension +
         2 * 2 * j * aggregate_size * leading_dimension;
    __m128 U_re;
    __m128 U_im;
    __m128 B_re;
