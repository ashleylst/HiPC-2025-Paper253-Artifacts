...
void coarse_operator_double_setup_vectorized(complex_double *operator, Level * l,
...
void set_coarse_neighbor_coupling_double_vectorized(complex_double *buffer1,
                                                    complex_double *buffer2, complex_double *V,
...
// fw and bw links have a symmetry that allows constructing one from another, see, e.g.,
// coarse_hopp_double for vectorization we store the operator for both cases, the "daggered" links
// need this transformed layout
...
static inline void coarse_hopp_double_vectorized(vector_double eta, vector_double phi,
                                                 OPERATOR_TYPE_double *D, Level *l) {
...
static inline void coarse_self_couplings_double_vectorized(vector_double eta, vector_double phi,
                                                           operator_double_struct *op, int start,
                                                           int end, Level *l) {
  ...
  OPERATOR_TYPE_double *clover =
      (g.n_flavours == 2) ? op->clover_doublet_vectorized : op->clover_vectorized;
#else
  ...
  for (int i = start; i < end; i++) {
    ...
    cgemv(site_size, clover + i * 2 * site_size * lda, lda, (double *)(phi + i * site_size),
          (double *)(eta + i * site_size));
  }
#endif
}

static inline void coarse_spinwise_hopp_double_vectorized(complex_double *eta1,
                                                          complex_double *eta2, complex_double *phi,
                                                          config_double D, int elements, Level *l) {
      ...
      for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
        for (int column = 0; column < num_eig_vect; column++) {
          ...
          for (int row = 0; row < num_eig_vect; row++) {
            ...
            D_re = _mm_set1_ps(real(D[column * num_eig_vect + row]));
...
static inline void coarse_spinwise_n_hopp_double_vectorized(complex_double *eta1,
                                                            complex_double *eta2,
                                                            complex_double *phi, config_double D,
                                                            int elements, Level *l) {
...
static inline void coarse_spinwise_n_daggered_hopp_double_vectorized(complex_double *eta1,
                                                                     complex_double *eta2,
                                                                     complex_double *phi,
                                                                     config_double D, int elements,
                                                                     Level *l) {
  ...
  for (int i = 0; i < elements; i += SIMD_LENGTH_double) {
    for (int column = 0; column < num_eig_vect; column++) {
      ...
      for (int row = 0; row < num_eig_vect; row++) {
        ...
        // load transpose
        D_re = _mm_set1_ps(real(D[row * num_eig_vect + column]));
